// <auto-generated />
using System;
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Infrastructure;
using Microsoft.EntityFrameworkCore.Metadata;
using Microsoft.EntityFrameworkCore.Migrations;
using Microsoft.EntityFrameworkCore.Storage.ValueConversion;
using NovaStay.Infrastructure.ContextDB;

#nullable disable

namespace NovaStay.Infrastructure.Migrations
{
    [DbContext(typeof(HostContext))]
    [Migration("20260629114314_AddManualIncomeReceipts")]
    partial class AddManualIncomeReceipts
    {
        /// <inheritdoc />
        protected override void BuildTargetModel(ModelBuilder modelBuilder)
        {
#pragma warning disable 612, 618
            modelBuilder
                .HasAnnotation("ProductVersion", "8.0.28")
                .HasAnnotation("Relational:MaxIdentifierLength", 128);

            SqlServerModelBuilderExtensions.UseIdentityColumns(modelBuilder);

            modelBuilder.Entity("NovaStay.Infrastructure.Models.Account", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier")
                        .HasDefaultValueSql("(newsequentialid())");

                    b.Property<string>("AccountType")
                        .IsRequired()
                        .HasMaxLength(30)
                        .IsUnicode(false)
                        .HasColumnType("varchar(30)");

                    b.Property<DateTime?>("CreatedAt")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("datetime")
                        .HasDefaultValueSql("(getdate())");

                    b.Property<string>("CustomerName")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<string>("Email")
                        .HasMaxLength(100)
                        .IsUnicode(false)
                        .HasColumnType("varchar(100)");

                    b.Property<bool?>("IsActive")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bit")
                        .HasDefaultValue(true);

                    b.Property<DateTime?>("LastLoginAt")
                        .HasColumnType("datetime");

                    b.Property<bool?>("MustSetPassword")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bit")
                        .HasDefaultValue(false);

                    b.Property<string>("PasswordHash")
                        .HasMaxLength(255)
                        .IsUnicode(false)
                        .HasColumnType("varchar(255)");

                    b.Property<DateTime?>("PasswordSetAt")
                        .HasColumnType("datetime");

                    b.Property<string>("Phone")
                        .IsRequired()
                        .HasMaxLength(15)
                        .IsUnicode(false)
                        .HasColumnType("varchar(15)");

                    b.HasKey("Id")
                        .HasName("PK_Accounts");

                    b.HasIndex(new[] { "Email" }, "IX_Accounts_Email");

                    b.HasIndex(new[] { "Phone" }, "IX_Accounts_Phone")
                        .IsUnique();

                    b.ToTable("Accounts");
                });

            modelBuilder.Entity("NovaStay.Infrastructure.Models.AccountRefreshToken", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier")
                        .HasDefaultValueSql("(newsequentialid())");

                    b.Property<Guid>("AccountId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTime?>("CreatedAt")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("datetime")
                        .HasDefaultValueSql("(getdate())");

                    b.Property<string>("CreatedByIp")
                        .HasMaxLength(45)
                        .IsUnicode(false)
                        .HasColumnType("varchar(45)");

                    b.Property<DateTime>("ExpiresAt")
                        .HasColumnType("datetime");

                    b.Property<string>("ReplacedByTokenHash")
                        .HasMaxLength(512)
                        .IsUnicode(false)
                        .HasColumnType("varchar(512)");

                    b.Property<DateTime?>("RevokedAt")
                        .HasColumnType("datetime");

                    b.Property<string>("RevokedByIp")
                        .HasMaxLength(45)
                        .IsUnicode(false)
                        .HasColumnType("varchar(45)");

                    b.Property<string>("TokenHash")
                        .IsRequired()
                        .HasMaxLength(512)
                        .IsUnicode(false)
                        .HasColumnType("varchar(512)");

                    b.HasKey("Id")
                        .HasName("PK_AccountRefreshTokens");

                    b.HasIndex(new[] { "AccountId", "ExpiresAt" }, "IX_AccountRefreshTokens_AccountId_ExpiresAt");

                    b.HasIndex(new[] { "TokenHash" }, "IX_AccountRefreshTokens_TokenHash")
                        .IsUnique();

                    b.ToTable("AccountRefreshTokens");
                });

            modelBuilder.Entity("NovaStay.Infrastructure.Models.Asset", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier")
                        .HasDefaultValueSql("(newsequentialid())");

                    b.Property<string>("AssetCode")
                        .HasMaxLength(50)
                        .IsUnicode(false)
                        .HasColumnType("varchar(50)");

                    b.Property<string>("AssetName")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<decimal?>("BaseValue")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("decimal(18, 2)")
                        .HasDefaultValue(0m);

                    b.Property<string>("Brand")
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<string>("Category")
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<DateTime?>("CreatedAt")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("datetime")
                        .HasDefaultValueSql("(getdate())");

                    b.Property<bool>("IsDeleted")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bit")
                        .HasDefaultValue(false);

                    b.Property<string>("Model")
                        .HasMaxLength(50)
                        .IsUnicode(false)
                        .HasColumnType("varchar(50)");

                    b.Property<Guid>("OrganizationId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateOnly?>("PurchaseDate")
                        .HasColumnType("date");

                    b.Property<DateOnly?>("WarrantyExpiryDate")
                        .HasColumnType("date");

                    b.HasKey("Id")
                        .HasName("PK__Assets__3214EC071B1A8B14");

                    b.HasIndex("OrganizationId");

                    b.ToTable("Assets");
                });

            modelBuilder.Entity("NovaStay.Infrastructure.Models.AssetAssignment", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier")
                        .HasDefaultValueSql("(newsequentialid())");

                    b.Property<Guid>("AssetId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTime?>("AssignedAt")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("datetime")
                        .HasDefaultValueSql("(getdate())");

                    b.Property<string>("Note")
                        .HasMaxLength(255)
                        .HasColumnType("nvarchar(255)");

                    b.Property<Guid>("OrganizationId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<Guid?>("RoomId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<string>("Status")
                        .ValueGeneratedOnAdd()
                        .HasMaxLength(20)
                        .IsUnicode(false)
                        .HasColumnType("varchar(20)")
                        .HasDefaultValue("Good");

                    b.HasKey("Id")
                        .HasName("PK__AssetAss__3214EC073B842B58");

                    b.HasIndex("AssetId");

                    b.HasIndex("OrganizationId");

                    b.HasIndex(new[] { "RoomId" }, "IX_AssetAssignments_RoomId");

                    b.ToTable("AssetAssignments");
                });

            modelBuilder.Entity("NovaStay.Infrastructure.Models.Booking", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier")
                        .HasDefaultValueSql("(newsequentialid())");

                    b.Property<decimal>("Amount")
                        .HasColumnType("decimal(18, 2)");

                    b.Property<string>("BookingType")
                        .IsRequired()
                        .HasMaxLength(20)
                        .IsUnicode(false)
                        .HasColumnType("varchar(20)");

                    b.Property<DateOnly>("CheckInDate")
                        .HasColumnType("date");

                    b.Property<DateOnly?>("CheckOutDate")
                        .HasColumnType("date");

                    b.Property<DateTime?>("CreatedAt")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("datetime")
                        .HasDefaultValueSql("(getdate())");

                    b.Property<string>("GuestEmail")
                        .HasMaxLength(100)
                        .IsUnicode(false)
                        .HasColumnType("varchar(100)");

                    b.Property<string>("GuestName")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<string>("GuestPhone")
                        .IsRequired()
                        .HasMaxLength(15)
                        .IsUnicode(false)
                        .HasColumnType("varchar(15)");

                    b.Property<Guid>("OrganizationId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<string>("PaymentStatus")
                        .ValueGeneratedOnAdd()
                        .HasMaxLength(20)
                        .IsUnicode(false)
                        .HasColumnType("varchar(20)")
                        .HasDefaultValue("Pending");

                    b.Property<string>("PaymentTransactionId")
                        .HasMaxLength(100)
                        .IsUnicode(false)
                        .HasColumnType("varchar(100)");

                    b.Property<Guid>("PropertyId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<Guid>("RoomId")
                        .HasColumnType("uniqueidentifier");

                    b.HasKey("Id")
                        .HasName("PK__Bookings__3214EC07E0378A72");

                    b.HasIndex("OrganizationId");

                    b.HasIndex("PropertyId");

                    b.HasIndex("RoomId");

                    b.ToTable("Bookings");
                });

            modelBuilder.Entity("NovaStay.Infrastructure.Models.Broker", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier")
                        .HasDefaultValueSql("(newsequentialid())");

                    b.Property<string>("FullName")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<Guid>("OrganizationId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<string>("Phone")
                        .IsRequired()
                        .HasMaxLength(15)
                        .IsUnicode(false)
                        .HasColumnType("varchar(15)");

                    b.Property<decimal?>("TotalCommissionEarned")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("decimal(18, 2)")
                        .HasDefaultValue(0m);

                    b.Property<decimal?>("WalletBalance")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("decimal(18, 2)")
                        .HasDefaultValue(0m);

                    b.HasKey("Id")
                        .HasName("PK__Brokers__3214EC07D3AA9FC2");

                    b.HasIndex("OrganizationId");

                    b.ToTable("Brokers");
                });

            modelBuilder.Entity("NovaStay.Infrastructure.Models.Contract", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier")
                        .HasDefaultValueSql("(newsequentialid())");

                    b.Property<Guid?>("BookingId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<decimal?>("BrokerCommission")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("decimal(18, 2)")
                        .HasDefaultValue(0m);

                    b.Property<Guid?>("BrokerId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<string>("CommissionStatus")
                        .ValueGeneratedOnAdd()
                        .HasMaxLength(20)
                        .IsUnicode(false)
                        .HasColumnType("varchar(20)")
                        .HasDefaultValue("Pending");

                    b.Property<string>("ContractPdfUrl")
                        .HasMaxLength(255)
                        .IsUnicode(false)
                        .HasColumnType("varchar(255)");

                    b.Property<DateTime?>("CreatedAt")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("datetime")
                        .HasDefaultValueSql("(getdate())");

                    b.Property<decimal>("DepositAmount")
                        .HasColumnType("decimal(18, 2)");

                    b.Property<DateOnly>("EndDate")
                        .HasColumnType("date");

                    b.Property<Guid>("OrganizationId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<Guid>("PropertyId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<Guid>("ResidentId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<Guid>("RoomId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateOnly>("StartDate")
                        .HasColumnType("date");

                    b.Property<string>("Status")
                        .IsRequired()
                        .HasMaxLength(20)
                        .IsUnicode(false)
                        .HasColumnType("varchar(20)");

                    b.HasKey("Id")
                        .HasName("PK__Contract__3214EC0707402DA0");

                    b.HasIndex("BookingId");

                    b.HasIndex("BrokerId");

                    b.HasIndex("OrganizationId");

                    b.HasIndex("PropertyId");

                    b.HasIndex("ResidentId");

                    b.HasIndex("RoomId");

                    b.ToTable("Contracts");
                });

            modelBuilder.Entity("NovaStay.Infrastructure.Models.Expense", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier")
                        .HasDefaultValueSql("(newsequentialid())");

                    b.Property<decimal>("Amount")
                        .HasColumnType("decimal(18, 2)");

                    b.Property<Guid?>("ApprovedByStaffUserId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTime?>("CreatedAt")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("datetime")
                        .HasDefaultValueSql("(getdate())");

                    b.Property<Guid?>("CreatedByStaffUserId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<string>("Description")
                        .HasMaxLength(500)
                        .HasColumnType("nvarchar(500)");

                    b.Property<Guid>("ExpenseCategoryId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<string>("ExpenseNumber")
                        .IsRequired()
                        .HasMaxLength(30)
                        .IsUnicode(false)
                        .HasColumnType("varchar(30)");

                    b.Property<string>("ExpenseType")
                        .IsRequired()
                        .HasMaxLength(20)
                        .IsUnicode(false)
                        .HasColumnType("varchar(20)");

                    b.Property<Guid>("OrganizationId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<string>("PayeeName")
                        .IsRequired()
                        .HasMaxLength(150)
                        .HasColumnType("nvarchar(150)");

                    b.Property<string>("PaymentMethod")
                        .IsRequired()
                        .HasMaxLength(20)
                        .IsUnicode(false)
                        .HasColumnType("varchar(20)");

                    b.Property<Guid?>("PropertyId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<string>("ReferenceCode")
                        .HasMaxLength(100)
                        .IsUnicode(false)
                        .HasColumnType("varchar(100)");

                    b.Property<Guid?>("RelatedBrokerId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<Guid?>("RelatedMaintenanceTicketId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<Guid?>("RoomId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTime>("SpentAt")
                        .HasColumnType("datetime");

                    b.Property<string>("Status")
                        .IsRequired()
                        .HasMaxLength(20)
                        .IsUnicode(false)
                        .HasColumnType("varchar(20)");

                    b.Property<DateTime?>("UpdatedAt")
                        .HasColumnType("datetime");

                    b.HasKey("Id")
                        .HasName("PK_Expenses");

                    b.HasIndex("ApprovedByStaffUserId");

                    b.HasIndex("CreatedByStaffUserId");

                    b.HasIndex("ExpenseCategoryId");

                    b.HasIndex("RelatedBrokerId");

                    b.HasIndex("RelatedMaintenanceTicketId");

                    b.HasIndex("RoomId");

                    b.HasIndex(new[] { "OrganizationId", "SpentAt" }, "IX_Expenses_OrganizationId_SpentAt");

                    b.HasIndex(new[] { "PropertyId", "ExpenseCategoryId", "SpentAt" }, "IX_Expenses_PropertyId_ExpenseCategoryId_SpentAt");

                    b.HasIndex(new[] { "OrganizationId", "ExpenseNumber" }, "UX_Expenses_OrganizationId_ExpenseNumber")
                        .IsUnique();

                    b.ToTable("Expenses");
                });

            modelBuilder.Entity("NovaStay.Infrastructure.Models.ExpenseCategory", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier")
                        .HasDefaultValueSql("(newsequentialid())");

                    b.Property<string>("CategoryCode")
                        .IsRequired()
                        .HasMaxLength(30)
                        .IsUnicode(false)
                        .HasColumnType("varchar(30)");

                    b.Property<string>("CategoryName")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<DateTime?>("CreatedAt")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("datetime")
                        .HasDefaultValueSql("(getdate())");

                    b.Property<bool>("IsActive")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bit")
                        .HasDefaultValue(true);

                    b.Property<Guid>("OrganizationId")
                        .HasColumnType("uniqueidentifier");

                    b.HasKey("Id")
                        .HasName("PK_ExpenseCategories");

                    b.HasIndex(new[] { "OrganizationId", "CategoryCode" }, "UX_ExpenseCategories_OrganizationId_CategoryCode")
                        .IsUnique();

                    b.ToTable("ExpenseCategories");
                });

            modelBuilder.Entity("NovaStay.Infrastructure.Models.IncomeCategory", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier")
                        .HasDefaultValueSql("(newsequentialid())");

                    b.Property<string>("CategoryCode")
                        .IsRequired()
                        .HasMaxLength(30)
                        .IsUnicode(false)
                        .HasColumnType("varchar(30)");

                    b.Property<string>("CategoryName")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<DateTime?>("CreatedAt")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("datetime")
                        .HasDefaultValueSql("(getdate())");

                    b.Property<bool>("IsActive")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bit")
                        .HasDefaultValue(true);

                    b.Property<Guid>("OrganizationId")
                        .HasColumnType("uniqueidentifier");

                    b.HasKey("Id")
                        .HasName("PK_IncomeCategories");

                    b.HasIndex(new[] { "OrganizationId", "CategoryCode" }, "UX_IncomeCategories_OrganizationId_CategoryCode")
                        .IsUnique();

                    b.ToTable("IncomeCategories");
                });

            modelBuilder.Entity("NovaStay.Infrastructure.Models.IncomeReceipt", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier")
                        .HasDefaultValueSql("(newsequentialid())");

                    b.Property<decimal>("Amount")
                        .HasColumnType("decimal(18, 2)");

                    b.Property<DateTime>("CollectedAt")
                        .HasColumnType("datetime");

                    b.Property<Guid?>("CollectedByStaffUserId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTime?>("CreatedAt")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("datetime")
                        .HasDefaultValueSql("(getdate())");

                    b.Property<string>("Description")
                        .HasMaxLength(500)
                        .HasColumnType("nvarchar(500)");

                    b.Property<Guid>("IncomeCategoryId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<string>("IncomeType")
                        .IsRequired()
                        .HasMaxLength(30)
                        .IsUnicode(false)
                        .HasColumnType("varchar(30)");

                    b.Property<Guid>("OrganizationId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<string>("PayerName")
                        .IsRequired()
                        .HasMaxLength(150)
                        .HasColumnType("nvarchar(150)");

                    b.Property<string>("PaymentMethod")
                        .IsRequired()
                        .HasMaxLength(20)
                        .IsUnicode(false)
                        .HasColumnType("varchar(20)");

                    b.Property<Guid>("PropertyId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<string>("ReceiptNumber")
                        .IsRequired()
                        .HasMaxLength(30)
                        .IsUnicode(false)
                        .HasColumnType("varchar(30)");

                    b.Property<string>("ReferenceCode")
                        .HasMaxLength(100)
                        .IsUnicode(false)
                        .HasColumnType("varchar(100)");

                    b.Property<Guid?>("ResidentId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<Guid?>("RoomId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<string>("Status")
                        .IsRequired()
                        .HasMaxLength(20)
                        .IsUnicode(false)
                        .HasColumnType("varchar(20)");

                    b.Property<DateTime?>("UpdatedAt")
                        .HasColumnType("datetime");

                    b.HasKey("Id")
                        .HasName("PK_IncomeReceipts");

                    b.HasIndex("CollectedByStaffUserId");

                    b.HasIndex("IncomeCategoryId");

                    b.HasIndex("ResidentId");

                    b.HasIndex("RoomId");

                    b.HasIndex(new[] { "OrganizationId", "CollectedAt" }, "IX_IncomeReceipts_OrganizationId_CollectedAt");

                    b.HasIndex(new[] { "PropertyId", "IncomeCategoryId", "CollectedAt" }, "IX_IncomeReceipts_PropertyId_IncomeCategoryId_CollectedAt");

                    b.HasIndex(new[] { "OrganizationId", "ReceiptNumber" }, "UX_IncomeReceipts_OrganizationId_ReceiptNumber")
                        .IsUnique();

                    b.ToTable("IncomeReceipts");
                });

            modelBuilder.Entity("NovaStay.Infrastructure.Models.Invoice", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier")
                        .HasDefaultValueSql("(newsequentialid())");

                    b.Property<decimal>("AdjustmentAmount")
                        .HasColumnType("decimal(18, 2)");

                    b.Property<Guid?>("BookingId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<Guid?>("ContractId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTime?>("CreatedAt")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("datetime")
                        .HasDefaultValueSql("(getdate())");

                    b.Property<decimal>("DiscountAmount")
                        .HasColumnType("decimal(18, 2)");

                    b.Property<DateOnly?>("DueDate")
                        .HasColumnType("date");

                    b.Property<string>("InvoiceNumber")
                        .HasMaxLength(30)
                        .IsUnicode(false)
                        .HasColumnType("varchar(30)");

                    b.Property<string>("InvoicePeriod")
                        .IsRequired()
                        .HasMaxLength(7)
                        .IsUnicode(false)
                        .HasColumnType("varchar(7)");

                    b.Property<string>("InvoiceType")
                        .HasMaxLength(30)
                        .IsUnicode(false)
                        .HasColumnType("varchar(30)");

                    b.Property<DateTime?>("IssuedAt")
                        .HasColumnType("datetime");

                    b.Property<decimal>("LateFeeAmount")
                        .HasColumnType("decimal(18, 2)");

                    b.Property<string>("Note")
                        .HasMaxLength(500)
                        .HasColumnType("nvarchar(500)");

                    b.Property<Guid>("OrganizationId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTime?>("PaidAt")
                        .HasColumnType("datetime");

                    b.Property<Guid?>("PropertyId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<string>("QrCodeUrl")
                        .HasMaxLength(500)
                        .IsUnicode(false)
                        .HasColumnType("varchar(500)");

                    b.Property<Guid?>("ResidentId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<Guid?>("RoomId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<decimal>("RoomPrice")
                        .HasColumnType("decimal(18, 2)");

                    b.Property<decimal>("ServicesPrice")
                        .HasColumnType("decimal(18, 2)");

                    b.Property<string>("Status")
                        .ValueGeneratedOnAdd()
                        .HasMaxLength(20)
                        .IsUnicode(false)
                        .HasColumnType("varchar(20)")
                        .HasDefaultValue("Unpaid");

                    b.Property<decimal>("Subtotal")
                        .HasColumnType("decimal(18, 2)");

                    b.Property<decimal>("TotalAmount")
                        .HasColumnType("decimal(18, 2)");

                    b.Property<DateTime?>("UpdatedAt")
                        .HasColumnType("datetime");

                    b.HasKey("Id")
                        .HasName("PK__Invoices__3214EC07023BFFC1");

                    b.HasIndex("BookingId");

                    b.HasIndex("ContractId");

                    b.HasIndex("RoomId");

                    b.HasIndex(new[] { "PropertyId", "RoomId", "InvoicePeriod" }, "IX_Invoices_PropertyId_RoomId_Period");

                    b.HasIndex(new[] { "ResidentId", "Status" }, "IX_Invoices_ResidentId_Status");

                    b.HasIndex(new[] { "Status", "InvoicePeriod" }, "IX_Invoices_Status_Period");

                    b.HasIndex(new[] { "OrganizationId", "InvoiceNumber" }, "UX_Invoices_OrganizationId_InvoiceNumber")
                        .IsUnique()
                        .HasFilter("[InvoiceNumber] IS NOT NULL");

                    b.ToTable("Invoices");
                });

            modelBuilder.Entity("NovaStay.Infrastructure.Models.InvoiceGenerationSchedule", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier")
                        .HasDefaultValueSql("(newsequentialid())");

                    b.Property<bool>("AutoSendToResident")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bit")
                        .HasDefaultValue(true);

                    b.Property<string>("BillingCycle")
                        .IsRequired()
                        .HasMaxLength(20)
                        .IsUnicode(false)
                        .HasColumnType("varchar(20)");

                    b.Property<DateTime?>("CreatedAt")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("datetime")
                        .HasDefaultValueSql("(getdate())");

                    b.Property<Guid?>("CreatedByStaffUserId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<int>("DueAfterDays")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasDefaultValue(7);

                    b.Property<int>("GenerateDayOfMonth")
                        .HasColumnType("int");

                    b.Property<TimeOnly>("GenerateTime")
                        .HasColumnType("time");

                    b.Property<bool>("IsActive")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bit")
                        .HasDefaultValue(true);

                    b.Property<DateTime?>("LastRunAt")
                        .HasColumnType("datetime");

                    b.Property<DateTime?>("NextRunAt")
                        .HasColumnType("datetime");

                    b.Property<Guid>("OrganizationId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<Guid?>("PropertyId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<string>("ScheduleName")
                        .IsRequired()
                        .HasMaxLength(150)
                        .HasColumnType("nvarchar(150)");

                    b.Property<string>("TimeZone")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<DateTime?>("UpdatedAt")
                        .HasColumnType("datetime");

                    b.HasKey("Id")
                        .HasName("PK_InvoiceGenerationSchedules");

                    b.HasIndex(new[] { "CreatedByStaffUserId" }, "IX_InvoiceGenerationSchedules_CreatedByStaffUserId");

                    b.HasIndex(new[] { "IsActive", "NextRunAt" }, "IX_InvoiceGenerationSchedules_IsActive_NextRunAt");

                    b.HasIndex(new[] { "OrganizationId" }, "IX_InvoiceGenerationSchedules_OrganizationId");

                    b.HasIndex(new[] { "PropertyId" }, "IX_InvoiceGenerationSchedules_PropertyId");

                    b.HasIndex(new[] { "OrganizationId", "PropertyId", "ScheduleName" }, "UX_InvoiceGenerationSchedules_OrganizationId_PropertyId_ScheduleName")
                        .IsUnique()
                        .HasFilter("[PropertyId] IS NOT NULL");

                    b.HasIndex(new[] { "OrganizationId", "ScheduleName" }, "UX_InvoiceGenerationSchedules_OrganizationId_ScheduleName")
                        .IsUnique()
                        .HasFilter("[PropertyId] IS NULL");

                    b.ToTable("InvoiceGenerationSchedules");
                });

            modelBuilder.Entity("NovaStay.Infrastructure.Models.InvoiceLine", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier")
                        .HasDefaultValueSql("(newsequentialid())");

                    b.Property<DateOnly?>("BillingEndDate")
                        .HasColumnType("date");

                    b.Property<DateOnly?>("BillingStartDate")
                        .HasColumnType("date");

                    b.Property<DateTime?>("CreatedAt")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("datetime")
                        .HasDefaultValueSql("(getdate())");

                    b.Property<string>("Description")
                        .IsRequired()
                        .HasMaxLength(255)
                        .HasColumnType("nvarchar(255)");

                    b.Property<int>("DisplayOrder")
                        .HasColumnType("int");

                    b.Property<Guid>("InvoiceId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<bool>("IsDebit")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bit")
                        .HasDefaultValue(true);

                    b.Property<decimal>("LineAmount")
                        .HasColumnType("decimal(18, 2)");

                    b.Property<string>("LineType")
                        .IsRequired()
                        .HasMaxLength(30)
                        .IsUnicode(false)
                        .HasColumnType("varchar(30)");

                    b.Property<decimal>("Quantity")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("decimal(18, 2)")
                        .HasDefaultValue(1m);

                    b.Property<Guid?>("SourceId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<string>("SourceType")
                        .HasMaxLength(30)
                        .IsUnicode(false)
                        .HasColumnType("varchar(30)");

                    b.Property<decimal>("UnitPrice")
                        .HasColumnType("decimal(18, 2)");

                    b.HasKey("Id")
                        .HasName("PK_InvoiceLines");

                    b.HasIndex(new[] { "InvoiceId", "DisplayOrder" }, "IX_InvoiceLines_InvoiceId_DisplayOrder");

                    b.ToTable("InvoiceLines");
                });

            modelBuilder.Entity("NovaStay.Infrastructure.Models.MaintenanceTicket", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier")
                        .HasDefaultValueSql("(newsequentialid())");

                    b.Property<Guid?>("AssetAssignmentId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<string>("Category")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<DateTime?>("CreatedAt")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("datetime")
                        .HasDefaultValueSql("(getdate())");

                    b.Property<string>("IncidentImageUrl")
                        .HasMaxLength(255)
                        .IsUnicode(false)
                        .HasColumnType("varchar(255)");

                    b.Property<Guid>("OrganizationId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<Guid?>("ResidentId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<string>("ResolvedImageUrl")
                        .HasMaxLength(255)
                        .IsUnicode(false)
                        .HasColumnType("varchar(255)");

                    b.Property<Guid>("RoomId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<string>("Status")
                        .ValueGeneratedOnAdd()
                        .HasMaxLength(20)
                        .IsUnicode(false)
                        .HasColumnType("varchar(20)")
                        .HasDefaultValue("Pending");

                    b.Property<Guid?>("TechnicianId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTime?>("UpdatedAt")
                        .HasColumnType("datetime");

                    b.Property<string>("UserDescription")
                        .IsRequired()
                        .HasMaxLength(500)
                        .HasColumnType("nvarchar(500)");

                    b.HasKey("Id")
                        .HasName("PK__Maintena__3214EC071D5966C6");

                    b.HasIndex("AssetAssignmentId");

                    b.HasIndex("OrganizationId");

                    b.HasIndex("ResidentId");

                    b.HasIndex("RoomId");

                    b.HasIndex("TechnicianId");

                    b.ToTable("MaintenanceTickets");
                });

            modelBuilder.Entity("NovaStay.Infrastructure.Models.Organization", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier")
                        .HasDefaultValueSql("(newsequentialid())");

                    b.Property<string>("BusinessArea")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<string>("BusinessName")
                        .IsRequired()
                        .HasMaxLength(150)
                        .HasColumnType("nvarchar(150)");

                    b.Property<DateTime?>("CreatedAt")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("datetime")
                        .HasDefaultValueSql("(getdate())");

                    b.Property<Guid>("OwnerAccountId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<string>("OwnerEmail")
                        .IsRequired()
                        .HasMaxLength(100)
                        .IsUnicode(false)
                        .HasColumnType("varchar(100)");

                    b.Property<string>("OwnerPhone")
                        .IsRequired()
                        .HasMaxLength(15)
                        .IsUnicode(false)
                        .HasColumnType("varchar(15)");

                    b.Property<Guid>("PackageId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<string>("SubscriptionStatus")
                        .IsRequired()
                        .HasMaxLength(20)
                        .IsUnicode(false)
                        .HasColumnType("varchar(20)");

                    b.Property<string>("TaxCode")
                        .HasMaxLength(20)
                        .IsUnicode(false)
                        .HasColumnType("varchar(20)");

                    b.Property<int?>("TokenBalance")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasDefaultValue(0);

                    b.Property<DateTime?>("UpdatedAt")
                        .HasColumnType("datetime");

                    b.HasKey("Id")
                        .HasName("PK__Organizations__3214EC070623D35C");

                    b.HasIndex(new[] { "OwnerAccountId" }, "IX_Organizations_OwnerAccountId");

                    b.HasIndex(new[] { "PackageId" }, "IX_Organizations_PackageId");

                    b.HasIndex(new[] { "OwnerEmail" }, "UQ__Organizations__FF0186BBAE6B6868")
                        .IsUnique();

                    b.ToTable("Organizations");
                });

            modelBuilder.Entity("NovaStay.Infrastructure.Models.PaymentReceipt", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier")
                        .HasDefaultValueSql("(newsequentialid())");

                    b.Property<decimal>("Amount")
                        .HasColumnType("decimal(18, 2)");

                    b.Property<Guid?>("CollectedByStaffUserId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTime?>("CreatedAt")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("datetime")
                        .HasDefaultValueSql("(getdate())");

                    b.Property<Guid>("InvoiceId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<string>("Note")
                        .HasMaxLength(500)
                        .HasColumnType("nvarchar(500)");

                    b.Property<Guid>("OrganizationId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTime>("PaidAt")
                        .HasColumnType("datetime");

                    b.Property<string>("PayerName")
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<string>("PaymentMethod")
                        .IsRequired()
                        .HasMaxLength(20)
                        .IsUnicode(false)
                        .HasColumnType("varchar(20)");

                    b.Property<Guid?>("PropertyId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<string>("ReceiptNumber")
                        .IsRequired()
                        .HasMaxLength(30)
                        .IsUnicode(false)
                        .HasColumnType("varchar(30)");

                    b.Property<string>("ReceiptType")
                        .IsRequired()
                        .HasMaxLength(20)
                        .IsUnicode(false)
                        .HasColumnType("varchar(20)");

                    b.Property<string>("ReferenceCode")
                        .HasMaxLength(100)
                        .IsUnicode(false)
                        .HasColumnType("varchar(100)");

                    b.Property<Guid?>("ResidentId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<string>("Status")
                        .IsRequired()
                        .HasMaxLength(20)
                        .IsUnicode(false)
                        .HasColumnType("varchar(20)");

                    b.HasKey("Id")
                        .HasName("PK_PaymentReceipts");

                    b.HasIndex("CollectedByStaffUserId");

                    b.HasIndex("PropertyId");

                    b.HasIndex("ResidentId");

                    b.HasIndex(new[] { "OrganizationId", "PaidAt" }, "IX_PaymentReceipts_OrganizationId_PaidAt");

                    b.HasIndex(new[] { "InvoiceId" }, "UX_PaymentReceipts_InvoiceId")
                        .IsUnique();

                    b.HasIndex(new[] { "OrganizationId", "ReceiptNumber" }, "UX_PaymentReceipts_OrganizationId_ReceiptNumber")
                        .IsUnique();

                    b.ToTable("PaymentReceipts");
                });

            modelBuilder.Entity("NovaStay.Infrastructure.Models.Permission", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier")
                        .HasDefaultValueSql("(newsequentialid())");

                    b.Property<string>("Module")
                        .IsRequired()
                        .HasMaxLength(50)
                        .IsUnicode(false)
                        .HasColumnType("varchar(50)");

                    b.Property<string>("PermissionKey")
                        .IsRequired()
                        .HasMaxLength(100)
                        .IsUnicode(false)
                        .HasColumnType("varchar(100)");

                    b.Property<string>("PermissionName")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.HasKey("Id")
                        .HasName("PK__Permissi__3214EC0720D981B2");

                    b.HasIndex(new[] { "PermissionKey" }, "UQ__Permissi__8884ABD46702546C")
                        .IsUnique();

                    b.ToTable("Permissions");
                });

            modelBuilder.Entity("NovaStay.Infrastructure.Models.Property", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier")
                        .HasDefaultValueSql("(newsequentialid())");

                    b.Property<string>("Address")
                        .IsRequired()
                        .HasMaxLength(255)
                        .HasColumnType("nvarchar(255)");

                    b.Property<DateTime?>("CreatedAt")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("datetime")
                        .HasDefaultValueSql("(getdate())");

                    b.Property<Guid>("OrganizationId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<string>("PropertyName")
                        .IsRequired()
                        .HasMaxLength(150)
                        .HasColumnType("nvarchar(150)");

                    b.Property<string>("PropertyType")
                        .IsRequired()
                        .HasMaxLength(20)
                        .IsUnicode(false)
                        .HasColumnType("varchar(20)");

                    b.Property<string>("Status")
                        .IsRequired()
                        .ValueGeneratedOnAdd()
                        .HasMaxLength(20)
                        .IsUnicode(false)
                        .HasColumnType("varchar(20)")
                        .HasDefaultValue("Active");

                    b.HasKey("Id")
                        .HasName("PK__Properti__3214EC07FE441D45");

                    b.HasIndex("OrganizationId");

                    b.ToTable("Properties");
                });

            modelBuilder.Entity("NovaStay.Infrastructure.Models.PropertyService", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier")
                        .HasDefaultValueSql("(newsequentialid())");

                    b.Property<string>("BillingCycle")
                        .HasMaxLength(30)
                        .IsUnicode(false)
                        .HasColumnType("varchar(30)");

                    b.Property<DateTime?>("CreatedAt")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("datetime")
                        .HasDefaultValueSql("(getdate())");

                    b.Property<decimal>("DefaultPrice")
                        .HasColumnType("decimal(18, 2)");

                    b.Property<string>("Description")
                        .HasMaxLength(500)
                        .HasColumnType("nvarchar(500)");

                    b.Property<bool>("IsActive")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bit")
                        .HasDefaultValue(true);

                    b.Property<Guid>("PropertyId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<string>("ServiceCode")
                        .HasMaxLength(50)
                        .IsUnicode(false)
                        .HasColumnType("varchar(50)");

                    b.Property<string>("ServiceName")
                        .IsRequired()
                        .HasMaxLength(150)
                        .HasColumnType("nvarchar(150)");

                    b.Property<string>("Unit")
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<DateTime?>("UpdatedAt")
                        .HasColumnType("datetime");

                    b.HasKey("Id")
                        .HasName("PK_PropertyServices");

                    b.HasIndex(new[] { "PropertyId", "IsActive" }, "IX_PropertyServices_PropertyId_IsActive");

                    b.HasIndex(new[] { "PropertyId", "ServiceName" }, "UX_PropertyServices_PropertyId_ServiceName")
                        .IsUnique();

                    b.ToTable("PropertyServices");
                });

            modelBuilder.Entity("NovaStay.Infrastructure.Models.Resident", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier")
                        .HasDefaultValueSql("(newsequentialid())");

                    b.Property<Guid>("AccountId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<string>("Address")
                        .HasMaxLength(255)
                        .HasColumnType("nvarchar(255)");

                    b.Property<DateTime?>("CreatedAt")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("datetime")
                        .HasDefaultValueSql("(getdate())");

                    b.Property<string>("Email")
                        .HasMaxLength(100)
                        .IsUnicode(false)
                        .HasColumnType("varchar(100)");

                    b.Property<string>("FullName")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<string>("IdBackImageUrl")
                        .HasMaxLength(255)
                        .IsUnicode(false)
                        .HasColumnType("varchar(255)");

                    b.Property<string>("IdFrontImageUrl")
                        .HasMaxLength(255)
                        .IsUnicode(false)
                        .HasColumnType("varchar(255)");

                    b.Property<string>("IdentityCardNumber")
                        .HasMaxLength(20)
                        .IsUnicode(false)
                        .HasColumnType("varchar(20)");

                    b.Property<string>("Phone")
                        .IsRequired()
                        .HasMaxLength(15)
                        .IsUnicode(false)
                        .HasColumnType("varchar(15)");

                    b.Property<string>("ProfileImageUrl")
                        .HasMaxLength(255)
                        .IsUnicode(false)
                        .HasColumnType("varchar(255)");

                    b.Property<string>("Sex")
                        .IsRequired()
                        .HasMaxLength(20)
                        .HasColumnType("nvarchar(20)");

                    b.HasKey("Id")
                        .HasName("PK__Resident__3214EC07115F9503");

                    b.HasIndex(new[] { "AccountId" }, "IX_Residents_AccountId")
                        .IsUnique();

                    b.ToTable("Residents");
                });

            modelBuilder.Entity("NovaStay.Infrastructure.Models.ResidentMembership", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier")
                        .HasDefaultValueSql("(newsequentialid())");

                    b.Property<Guid>("AccountId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTime?>("ActivatedAt")
                        .HasColumnType("datetime");

                    b.Property<DateTime?>("CreatedAt")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("datetime")
                        .HasDefaultValueSql("(getdate())");

                    b.Property<DateTime?>("InvitedAt")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("datetime")
                        .HasDefaultValueSql("(getdate())");

                    b.Property<DateTime?>("JoinedAt")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("datetime")
                        .HasDefaultValueSql("(getdate())");

                    b.Property<string>("MembershipCode")
                        .IsRequired()
                        .HasMaxLength(20)
                        .IsUnicode(false)
                        .HasColumnType("varchar(20)");

                    b.Property<Guid>("OrganizationId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<Guid>("ResidentId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTime?>("RespondedAt")
                        .HasColumnType("datetime");

                    b.Property<string>("Status")
                        .IsRequired()
                        .ValueGeneratedOnAdd()
                        .HasMaxLength(20)
                        .IsUnicode(false)
                        .HasColumnType("varchar(20)")
                        .HasDefaultValue("Pending");

                    b.HasKey("Id")
                        .HasName("PK_ResidentMemberships");

                    b.HasIndex(new[] { "AccountId" }, "IX_ResidentMemberships_AccountId");

                    b.HasIndex(new[] { "OrganizationId" }, "IX_ResidentMemberships_OrganizationId");

                    b.HasIndex(new[] { "ResidentId" }, "IX_ResidentMemberships_ResidentId");

                    b.HasIndex(new[] { "AccountId", "OrganizationId" }, "UX_ResidentMemberships_AccountId_OrganizationId")
                        .IsUnique();

                    b.HasIndex(new[] { "MembershipCode" }, "UX_ResidentMemberships_MembershipCode")
                        .IsUnique();

                    b.ToTable("ResidentMemberships");
                });

            modelBuilder.Entity("NovaStay.Infrastructure.Models.Role", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier")
                        .HasDefaultValueSql("(newsequentialid())");

                    b.Property<DateTime?>("CreatedAt")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("datetime")
                        .HasDefaultValueSql("(getdate())");

                    b.Property<string>("Description")
                        .HasMaxLength(255)
                        .HasColumnType("nvarchar(255)");

                    b.Property<Guid>("OrganizationId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<string>("RoleKey")
                        .IsRequired()
                        .HasMaxLength(50)
                        .IsUnicode(false)
                        .HasColumnType("varchar(50)");

                    b.Property<string>("RoleName")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.HasKey("Id")
                        .HasName("PK__Roles__3214EC07BE72DF6E");

                    b.HasIndex("OrganizationId");

                    b.ToTable("Roles");
                });

            modelBuilder.Entity("NovaStay.Infrastructure.Models.Room", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier")
                        .HasDefaultValueSql("(newsequentialid())");

                    b.Property<string>("AmenitiesJson")
                        .HasColumnType("nvarchar(max)");

                    b.Property<decimal>("BasePrice")
                        .HasColumnType("decimal(18, 2)");

                    b.Property<DateTime?>("CreatedAt")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("datetime")
                        .HasDefaultValueSql("(getdate())");

                    b.Property<int>("Floor")
                        .HasColumnType("int");

                    b.Property<bool>("IsDeleted")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bit")
                        .HasDefaultValue(false);

                    b.Property<int?>("MaxOccupants")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasDefaultValue(1);

                    b.Property<Guid>("PropertyId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<string>("RoomNumber")
                        .IsRequired()
                        .HasMaxLength(20)
                        .IsUnicode(false)
                        .HasColumnType("varchar(20)");

                    b.Property<byte[]>("RowVersion")
                        .IsConcurrencyToken()
                        .IsRequired()
                        .ValueGeneratedOnAddOrUpdate()
                        .HasColumnType("rowversion");

                    b.Property<string>("Status")
                        .IsRequired()
                        .HasMaxLength(20)
                        .IsUnicode(false)
                        .HasColumnType("varchar(20)");

                    b.HasKey("Id")
                        .HasName("PK__Rooms__3214EC077FA99C97");

                    b.HasIndex(new[] { "PropertyId", "Status" }, "IX_Rooms_PropertyId_Status");

                    b.ToTable("Rooms");
                });

            modelBuilder.Entity("NovaStay.Infrastructure.Models.RoomAvailability", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier")
                        .HasDefaultValueSql("(newsequentialid())");

                    b.Property<decimal>("DynamicPrice")
                        .HasColumnType("decimal(18, 2)");

                    b.Property<bool?>("IsBooked")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bit")
                        .HasDefaultValue(false);

                    b.Property<Guid>("RoomId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateOnly>("StayDate")
                        .HasColumnType("date");

                    b.HasKey("Id")
                        .HasName("PK__RoomAvai__3214EC07207FCE01");

                    b.HasIndex("RoomId");

                    b.ToTable("RoomAvailabilities");
                });

            modelBuilder.Entity("NovaStay.Infrastructure.Models.RoomImage", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier")
                        .HasDefaultValueSql("(newsequentialid())");

                    b.Property<string>("ImageUrl")
                        .IsRequired()
                        .HasMaxLength(255)
                        .IsUnicode(false)
                        .HasColumnType("varchar(255)");

                    b.Property<bool?>("IsCover")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bit")
                        .HasDefaultValue(false);

                    b.Property<Guid>("RoomId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTime?>("UploadedAt")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("datetime")
                        .HasDefaultValueSql("(getdate())");

                    b.HasKey("Id")
                        .HasName("PK__RoomImag__3214EC079F9973C0");

                    b.HasIndex(new[] { "RoomId" }, "IX_RoomImages_RoomId");

                    b.ToTable("RoomImages");
                });

            modelBuilder.Entity("NovaStay.Infrastructure.Models.RoomService", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier")
                        .HasDefaultValueSql("(newsequentialid())");

                    b.Property<DateTime?>("AssignedAt")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("datetime")
                        .HasDefaultValueSql("(getdate())");

                    b.Property<bool>("IsActive")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bit")
                        .HasDefaultValue(true);

                    b.Property<string>("Note")
                        .HasMaxLength(255)
                        .HasColumnType("nvarchar(255)");

                    b.Property<decimal?>("PriceOverride")
                        .HasColumnType("decimal(18, 2)");

                    b.Property<Guid>("PropertyServiceId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTime?>("RemovedAt")
                        .HasColumnType("datetime");

                    b.Property<Guid>("RoomId")
                        .HasColumnType("uniqueidentifier");

                    b.HasKey("Id")
                        .HasName("PK_RoomServices");

                    b.HasIndex(new[] { "PropertyServiceId" }, "IX_RoomServices_PropertyServiceId");

                    b.HasIndex(new[] { "RoomId" }, "IX_RoomServices_RoomId");

                    b.HasIndex(new[] { "RoomId", "PropertyServiceId" }, "UX_RoomServices_RoomId_PropertyServiceId")
                        .IsUnique();

                    b.ToTable("RoomServices");
                });

            modelBuilder.Entity("NovaStay.Infrastructure.Models.StaffUser", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier")
                        .HasDefaultValueSql("(newsequentialid())");

                    b.Property<Guid>("AccountId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTime?>("CreatedAt")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("datetime")
                        .HasDefaultValueSql("(getdate())");

                    b.Property<string>("FullName")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<bool?>("IsActive")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bit")
                        .HasDefaultValue(true);

                    b.Property<Guid>("OrganizationId")
                        .HasColumnType("uniqueidentifier");

                    b.HasKey("Id")
                        .HasName("PK__Users__3214EC078909D83A");

                    b.HasIndex(new[] { "AccountId" }, "IX_StaffUsers_AccountId")
                        .IsUnique();

                    b.HasIndex(new[] { "OrganizationId" }, "IX_Users_OrganizationId");

                    b.ToTable("StaffUsers");
                });

            modelBuilder.Entity("NovaStay.Infrastructure.Models.SubscriptionPackage", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier")
                        .HasDefaultValueSql("(newsequentialid())");

                    b.Property<bool?>("AllowAiFeatures")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bit")
                        .HasDefaultValue(false);

                    b.Property<bool?>("AllowCustomRoles")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bit")
                        .HasDefaultValue(false);

                    b.Property<DateTime?>("CreatedAt")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("datetime")
                        .HasDefaultValueSql("(getdate())");

                    b.Property<int>("MaxProperties")
                        .HasColumnType("int");

                    b.Property<int>("MaxRooms")
                        .HasColumnType("int");

                    b.Property<string>("PackageKey")
                        .IsRequired()
                        .HasMaxLength(20)
                        .IsUnicode(false)
                        .HasColumnType("varchar(20)");

                    b.Property<string>("PackageName")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<decimal>("PriceMonthly")
                        .HasColumnType("decimal(18, 2)");

                    b.Property<int?>("TokenGiftMonthly")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasDefaultValue(0);

                    b.HasKey("Id")
                        .HasName("PK__Subscrip__3214EC077D1660D7");

                    b.HasIndex(new[] { "PackageKey" }, "UQ__Subscrip__63953E72B6E7DAB9")
                        .IsUnique();

                    b.ToTable("SubscriptionPackages");
                });

            modelBuilder.Entity("NovaStay.Infrastructure.Models.Technician", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier")
                        .HasDefaultValueSql("(newsequentialid())");

                    b.Property<string>("FullName")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<bool?>("IsAvailable")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bit")
                        .HasDefaultValue(true);

                    b.Property<Guid>("OrganizationId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<string>("Phone")
                        .IsRequired()
                        .HasMaxLength(15)
                        .IsUnicode(false)
                        .HasColumnType("varchar(15)");

                    b.Property<string>("Specialty")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.HasKey("Id")
                        .HasName("PK__Technici__3214EC0701E593D1");

                    b.HasIndex("OrganizationId");

                    b.ToTable("Technicians");
                });

            modelBuilder.Entity("NovaStay.Infrastructure.Models.UtilityMeter", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier")
                        .HasDefaultValueSql("(newsequentialid())");

                    b.Property<DateTime?>("CreatedAt")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("datetime")
                        .HasDefaultValueSql("(getdate())");

                    b.Property<decimal>("InitialReading")
                        .HasColumnType("decimal(18, 3)");

                    b.Property<DateTime?>("InstalledAt")
                        .HasColumnType("datetime");

                    b.Property<bool>("IsActive")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bit")
                        .HasDefaultValue(true);

                    b.Property<string>("MeterCode")
                        .HasMaxLength(50)
                        .IsUnicode(false)
                        .HasColumnType("varchar(50)");

                    b.Property<string>("MeterType")
                        .IsRequired()
                        .HasMaxLength(30)
                        .IsUnicode(false)
                        .HasColumnType("varchar(30)");

                    b.Property<Guid>("PropertyServiceId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<Guid>("RoomId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<string>("Unit")
                        .IsRequired()
                        .HasMaxLength(20)
                        .HasColumnType("nvarchar(20)");

                    b.Property<DateTime?>("UpdatedAt")
                        .HasColumnType("datetime");

                    b.HasKey("Id")
                        .HasName("PK_UtilityMeters");

                    b.HasIndex(new[] { "PropertyServiceId" }, "IX_UtilityMeters_PropertyServiceId");

                    b.HasIndex(new[] { "RoomId" }, "IX_UtilityMeters_RoomId");

                    b.HasIndex(new[] { "MeterCode" }, "UX_UtilityMeters_MeterCode")
                        .IsUnique()
                        .HasFilter("[MeterCode] IS NOT NULL");

                    b.HasIndex(new[] { "RoomId", "PropertyServiceId" }, "UX_UtilityMeters_RoomId_PropertyServiceId")
                        .IsUnique();

                    b.ToTable("UtilityMeters");
                });

            modelBuilder.Entity("NovaStay.Infrastructure.Models.UtilityReading", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier")
                        .HasDefaultValueSql("(newsequentialid())");

                    b.Property<decimal>("Amount")
                        .HasColumnType("decimal(18, 2)");

                    b.Property<string>("BillingPeriod")
                        .IsRequired()
                        .HasMaxLength(7)
                        .IsUnicode(false)
                        .HasColumnType("varchar(7)");

                    b.Property<decimal>("Consumption")
                        .HasColumnType("decimal(18, 3)");

                    b.Property<DateTime?>("CreatedAt")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("datetime")
                        .HasDefaultValueSql("(getdate())");

                    b.Property<decimal>("CurrentReading")
                        .HasColumnType("decimal(18, 3)");

                    b.Property<string>("Note")
                        .HasMaxLength(500)
                        .HasColumnType("nvarchar(500)");

                    b.Property<decimal>("PreviousReading")
                        .HasColumnType("decimal(18, 3)");

                    b.Property<DateTime>("ReadingDate")
                        .HasColumnType("datetime");

                    b.Property<Guid?>("RecordedByStaffUserId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<string>("Status")
                        .IsRequired()
                        .ValueGeneratedOnAdd()
                        .HasMaxLength(20)
                        .IsUnicode(false)
                        .HasColumnType("varchar(20)")
                        .HasDefaultValue("Draft");

                    b.Property<decimal>("UnitPrice")
                        .HasColumnType("decimal(18, 2)");

                    b.Property<Guid>("UtilityMeterId")
                        .HasColumnType("uniqueidentifier");

                    b.HasKey("Id")
                        .HasName("PK_UtilityReadings");

                    b.HasIndex(new[] { "RecordedByStaffUserId" }, "IX_UtilityReadings_RecordedByStaffUserId");

                    b.HasIndex(new[] { "Status", "BillingPeriod" }, "IX_UtilityReadings_Status_BillingPeriod");

                    b.HasIndex(new[] { "UtilityMeterId", "BillingPeriod" }, "UX_UtilityReadings_UtilityMeterId_BillingPeriod")
                        .IsUnique();

                    b.ToTable("UtilityReadings");
                });

            modelBuilder.Entity("NovaStay.Infrastructure.Models.UtilityTariff", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier")
                        .HasDefaultValueSql("(newsequentialid())");

                    b.Property<DateTime?>("CreatedAt")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("datetime")
                        .HasDefaultValueSql("(getdate())");

                    b.Property<DateTime>("EffectiveFrom")
                        .HasColumnType("datetime");

                    b.Property<DateTime?>("EffectiveTo")
                        .HasColumnType("datetime");

                    b.Property<string>("PricingMode")
                        .IsRequired()
                        .HasMaxLength(20)
                        .IsUnicode(false)
                        .HasColumnType("varchar(20)");

                    b.Property<Guid>("PropertyServiceId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<decimal>("UnitPrice")
                        .HasColumnType("decimal(18, 2)");

                    b.HasKey("Id")
                        .HasName("PK_UtilityTariffs");

                    b.HasIndex(new[] { "PropertyServiceId", "EffectiveFrom" }, "UX_UtilityTariffs_PropertyServiceId_EffectiveFrom")
                        .IsUnique();

                    b.ToTable("UtilityTariffs");
                });

            modelBuilder.Entity("RolePermission", b =>
                {
                    b.Property<Guid>("RoleId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<Guid>("PermissionId")
                        .HasColumnType("uniqueidentifier");

                    b.HasKey("RoleId", "PermissionId")
                        .HasName("PK__RolePerm__6400A1A8CB2E4434");

                    b.HasIndex("PermissionId");

                    b.ToTable("RolePermissions", (string)null);
                });

            modelBuilder.Entity("StaffUserPropertyMapping", b =>
                {
                    b.Property<Guid>("StaffUserId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<Guid>("PropertyId")
                        .HasColumnType("uniqueidentifier");

                    b.HasKey("StaffUserId", "PropertyId")
                        .HasName("PK__UserProp__5084563FC247FEEC");

                    b.HasIndex("PropertyId");

                    b.ToTable("StaffUserPropertyMapping", (string)null);
                });

            modelBuilder.Entity("StaffUserRole", b =>
                {
                    b.Property<Guid>("StaffUserId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<Guid>("RoleId")
                        .HasColumnType("uniqueidentifier");

                    b.HasKey("StaffUserId", "RoleId")
                        .HasName("PK__UserRole__AF2760AD29238DE0");

                    b.HasIndex("RoleId");

                    b.ToTable("StaffUserRoles", (string)null);
                });

            modelBuilder.Entity("NovaStay.Infrastructure.Models.AccountRefreshToken", b =>
                {
                    b.HasOne("NovaStay.Infrastructure.Models.Account", "Account")
                        .WithMany("AccountRefreshTokens")
                        .HasForeignKey("AccountId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("FK_AccountRefreshTokens_Accounts_AccountId");

                    b.Navigation("Account");
                });

            modelBuilder.Entity("NovaStay.Infrastructure.Models.Asset", b =>
                {
                    b.HasOne("NovaStay.Infrastructure.Models.Organization", "Organization")
                        .WithMany("Assets")
                        .HasForeignKey("OrganizationId")
                        .IsRequired()
                        .HasConstraintName("FK__Assets__OrganizationId__02FC7413");

                    b.Navigation("Organization");
                });

            modelBuilder.Entity("NovaStay.Infrastructure.Models.AssetAssignment", b =>
                {
                    b.HasOne("NovaStay.Infrastructure.Models.Asset", "Asset")
                        .WithMany("AssetAssignments")
                        .HasForeignKey("AssetId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("FK__AssetAssi__Asset__08B54D69");

                    b.HasOne("NovaStay.Infrastructure.Models.Organization", "Organization")
                        .WithMany("AssetAssignments")
                        .HasForeignKey("OrganizationId")
                        .IsRequired()
                        .HasConstraintName("FK__AssetAssi__Tenan__07C12930");

                    b.HasOne("NovaStay.Infrastructure.Models.Room", "Room")
                        .WithMany("AssetAssignments")
                        .HasForeignKey("RoomId")
                        .HasConstraintName("FK__AssetAssi__RoomI__09A971A2");

                    b.Navigation("Asset");

                    b.Navigation("Organization");

                    b.Navigation("Room");
                });

            modelBuilder.Entity("NovaStay.Infrastructure.Models.Booking", b =>
                {
                    b.HasOne("NovaStay.Infrastructure.Models.Organization", "Organization")
                        .WithMany("Bookings")
                        .HasForeignKey("OrganizationId")
                        .IsRequired()
                        .HasConstraintName("FK__Bookings__Organization__1BC821DD");

                    b.HasOne("NovaStay.Infrastructure.Models.Property", "Property")
                        .WithMany("Bookings")
                        .HasForeignKey("PropertyId")
                        .IsRequired()
                        .HasConstraintName("FK__Bookings__Proper__1CBC4616");

                    b.HasOne("NovaStay.Infrastructure.Models.Room", "Room")
                        .WithMany("Bookings")
                        .HasForeignKey("RoomId")
                        .IsRequired()
                        .HasConstraintName("FK__Bookings__RoomId__1DB06A4F");

                    b.Navigation("Organization");

                    b.Navigation("Property");

                    b.Navigation("Room");
                });

            modelBuilder.Entity("NovaStay.Infrastructure.Models.Broker", b =>
                {
                    b.HasOne("NovaStay.Infrastructure.Models.Organization", "Organization")
                        .WithMany("Brokers")
                        .HasForeignKey("OrganizationId")
                        .IsRequired()
                        .HasConstraintName("FK__Brokers__OrganizationI__2739D489");

                    b.Navigation("Organization");
                });

            modelBuilder.Entity("NovaStay.Infrastructure.Models.Contract", b =>
                {
                    b.HasOne("NovaStay.Infrastructure.Models.Booking", "Booking")
                        .WithMany("Contracts")
                        .HasForeignKey("BookingId")
                        .HasConstraintName("FK__Contracts__Booki__32AB8735");

                    b.HasOne("NovaStay.Infrastructure.Models.Broker", "Broker")
                        .WithMany("Contracts")
                        .HasForeignKey("BrokerId")
                        .HasConstraintName("FK__Contracts__Broke__31B762FC");

                    b.HasOne("NovaStay.Infrastructure.Models.Organization", "Organization")
                        .WithMany("Contracts")
                        .HasForeignKey("OrganizationId")
                        .IsRequired()
                        .HasConstraintName("FK__Contracts__Tenan__2CF2ADDF");

                    b.HasOne("NovaStay.Infrastructure.Models.Property", "Property")
                        .WithMany("Contracts")
                        .HasForeignKey("PropertyId")
                        .IsRequired()
                        .HasConstraintName("FK__Contracts__Prope__2DE6D218");

                    b.HasOne("NovaStay.Infrastructure.Models.Resident", "Resident")
                        .WithMany("Contracts")
                        .HasForeignKey("ResidentId")
                        .IsRequired()
                        .HasConstraintName("FK__Contracts__Resid__30C33EC3");

                    b.HasOne("NovaStay.Infrastructure.Models.Room", "Room")
                        .WithMany("Contracts")
                        .HasForeignKey("RoomId")
                        .IsRequired()
                        .HasConstraintName("FK__Contracts__RoomI__2EDAF651");

                    b.Navigation("Booking");

                    b.Navigation("Broker");

                    b.Navigation("Organization");

                    b.Navigation("Property");

                    b.Navigation("Resident");

                    b.Navigation("Room");
                });

            modelBuilder.Entity("NovaStay.Infrastructure.Models.Expense", b =>
                {
                    b.HasOne("NovaStay.Infrastructure.Models.StaffUser", "ApprovedByStaffUser")
                        .WithMany("ApprovedExpenses")
                        .HasForeignKey("ApprovedByStaffUserId")
                        .HasConstraintName("FK_Expenses_StaffUsers_ApprovedByStaffUserId");

                    b.HasOne("NovaStay.Infrastructure.Models.StaffUser", "CreatedByStaffUser")
                        .WithMany("CreatedExpenses")
                        .HasForeignKey("CreatedByStaffUserId")
                        .HasConstraintName("FK_Expenses_StaffUsers_CreatedByStaffUserId");

                    b.HasOne("NovaStay.Infrastructure.Models.ExpenseCategory", "ExpenseCategory")
                        .WithMany("Expenses")
                        .HasForeignKey("ExpenseCategoryId")
                        .IsRequired()
                        .HasConstraintName("FK_Expenses_ExpenseCategories_ExpenseCategoryId");

                    b.HasOne("NovaStay.Infrastructure.Models.Organization", "Organization")
                        .WithMany("Expenses")
                        .HasForeignKey("OrganizationId")
                        .IsRequired()
                        .HasConstraintName("FK_Expenses_Organizations_OrganizationId");

                    b.HasOne("NovaStay.Infrastructure.Models.Property", "Property")
                        .WithMany("Expenses")
                        .HasForeignKey("PropertyId")
                        .HasConstraintName("FK_Expenses_Properties_PropertyId");

                    b.HasOne("NovaStay.Infrastructure.Models.Broker", "RelatedBroker")
                        .WithMany("Expenses")
                        .HasForeignKey("RelatedBrokerId")
                        .HasConstraintName("FK_Expenses_Brokers_RelatedBrokerId");

                    b.HasOne("NovaStay.Infrastructure.Models.MaintenanceTicket", "RelatedMaintenanceTicket")
                        .WithMany("Expenses")
                        .HasForeignKey("RelatedMaintenanceTicketId")
                        .HasConstraintName("FK_Expenses_MaintenanceTickets_RelatedMaintenanceTicketId");

                    b.HasOne("NovaStay.Infrastructure.Models.Room", "Room")
                        .WithMany("Expenses")
                        .HasForeignKey("RoomId")
                        .HasConstraintName("FK_Expenses_Rooms_RoomId");

                    b.Navigation("ApprovedByStaffUser");

                    b.Navigation("CreatedByStaffUser");

                    b.Navigation("ExpenseCategory");

                    b.Navigation("Organization");

                    b.Navigation("Property");

                    b.Navigation("RelatedBroker");

                    b.Navigation("RelatedMaintenanceTicket");

                    b.Navigation("Room");
                });

            modelBuilder.Entity("NovaStay.Infrastructure.Models.ExpenseCategory", b =>
                {
                    b.HasOne("NovaStay.Infrastructure.Models.Organization", "Organization")
                        .WithMany("ExpenseCategories")
                        .HasForeignKey("OrganizationId")
                        .IsRequired()
                        .HasConstraintName("FK_ExpenseCategories_Organizations_OrganizationId");

                    b.Navigation("Organization");
                });

            modelBuilder.Entity("NovaStay.Infrastructure.Models.IncomeCategory", b =>
                {
                    b.HasOne("NovaStay.Infrastructure.Models.Organization", "Organization")
                        .WithMany("IncomeCategories")
                        .HasForeignKey("OrganizationId")
                        .IsRequired()
                        .HasConstraintName("FK_IncomeCategories_Organizations_OrganizationId");

                    b.Navigation("Organization");
                });

            modelBuilder.Entity("NovaStay.Infrastructure.Models.IncomeReceipt", b =>
                {
                    b.HasOne("NovaStay.Infrastructure.Models.StaffUser", "CollectedByStaffUser")
                        .WithMany("CollectedIncomeReceipts")
                        .HasForeignKey("CollectedByStaffUserId")
                        .HasConstraintName("FK_IncomeReceipts_StaffUsers_CollectedByStaffUserId");

                    b.HasOne("NovaStay.Infrastructure.Models.IncomeCategory", "IncomeCategory")
                        .WithMany("IncomeReceipts")
                        .HasForeignKey("IncomeCategoryId")
                        .IsRequired()
                        .HasConstraintName("FK_IncomeReceipts_IncomeCategories_IncomeCategoryId");

                    b.HasOne("NovaStay.Infrastructure.Models.Organization", "Organization")
                        .WithMany("IncomeReceipts")
                        .HasForeignKey("OrganizationId")
                        .IsRequired()
                        .HasConstraintName("FK_IncomeReceipts_Organizations_OrganizationId");

                    b.HasOne("NovaStay.Infrastructure.Models.Property", "Property")
                        .WithMany("IncomeReceipts")
                        .HasForeignKey("PropertyId")
                        .IsRequired()
                        .HasConstraintName("FK_IncomeReceipts_Properties_PropertyId");

                    b.HasOne("NovaStay.Infrastructure.Models.Resident", "Resident")
                        .WithMany("IncomeReceipts")
                        .HasForeignKey("ResidentId")
                        .HasConstraintName("FK_IncomeReceipts_Residents_ResidentId");

                    b.HasOne("NovaStay.Infrastructure.Models.Room", "Room")
                        .WithMany()
                        .HasForeignKey("RoomId")
                        .HasConstraintName("FK_IncomeReceipts_Rooms_RoomId");

                    b.Navigation("CollectedByStaffUser");

                    b.Navigation("IncomeCategory");

                    b.Navigation("Organization");

                    b.Navigation("Property");

                    b.Navigation("Resident");

                    b.Navigation("Room");
                });

            modelBuilder.Entity("NovaStay.Infrastructure.Models.Invoice", b =>
                {
                    b.HasOne("NovaStay.Infrastructure.Models.Booking", "Booking")
                        .WithMany("Invoices")
                        .HasForeignKey("BookingId")
                        .HasConstraintName("FK__Invoices__Bookin__395884C4");

                    b.HasOne("NovaStay.Infrastructure.Models.Contract", "Contract")
                        .WithMany("Invoices")
                        .HasForeignKey("ContractId")
                        .HasConstraintName("FK__Invoices__Contra__3864608B");

                    b.HasOne("NovaStay.Infrastructure.Models.Organization", "Organization")
                        .WithMany("Invoices")
                        .HasForeignKey("OrganizationId")
                        .IsRequired()
                        .HasConstraintName("FK__Invoices__Organization__37703C52");

                    b.HasOne("NovaStay.Infrastructure.Models.Property", "Property")
                        .WithMany("Invoices")
                        .HasForeignKey("PropertyId")
                        .HasConstraintName("FK_Invoices_Properties_PropertyId");

                    b.HasOne("NovaStay.Infrastructure.Models.Resident", "Resident")
                        .WithMany("Invoices")
                        .HasForeignKey("ResidentId")
                        .HasConstraintName("FK_Invoices_Residents_ResidentId");

                    b.HasOne("NovaStay.Infrastructure.Models.Room", "Room")
                        .WithMany("Invoices")
                        .HasForeignKey("RoomId")
                        .HasConstraintName("FK_Invoices_Rooms_RoomId");

                    b.Navigation("Booking");

                    b.Navigation("Contract");

                    b.Navigation("Organization");

                    b.Navigation("Property");

                    b.Navigation("Resident");

                    b.Navigation("Room");
                });

            modelBuilder.Entity("NovaStay.Infrastructure.Models.InvoiceGenerationSchedule", b =>
                {
                    b.HasOne("NovaStay.Infrastructure.Models.StaffUser", "CreatedByStaffUser")
                        .WithMany("InvoiceGenerationSchedules")
                        .HasForeignKey("CreatedByStaffUserId")
                        .HasConstraintName("FK_InvoiceGenerationSchedules_StaffUsers_CreatedByStaffUserId");

                    b.HasOne("NovaStay.Infrastructure.Models.Organization", "Organization")
                        .WithMany("InvoiceGenerationSchedules")
                        .HasForeignKey("OrganizationId")
                        .IsRequired()
                        .HasConstraintName("FK_InvoiceGenerationSchedules_Organizations_OrganizationId");

                    b.HasOne("NovaStay.Infrastructure.Models.Property", "Property")
                        .WithMany("InvoiceGenerationSchedules")
                        .HasForeignKey("PropertyId")
                        .HasConstraintName("FK_InvoiceGenerationSchedules_Properties_PropertyId");

                    b.Navigation("CreatedByStaffUser");

                    b.Navigation("Organization");

                    b.Navigation("Property");
                });

            modelBuilder.Entity("NovaStay.Infrastructure.Models.InvoiceLine", b =>
                {
                    b.HasOne("NovaStay.Infrastructure.Models.Invoice", "Invoice")
                        .WithMany("InvoiceLines")
                        .HasForeignKey("InvoiceId")
                        .IsRequired()
                        .HasConstraintName("FK_InvoiceLines_Invoices_InvoiceId");

                    b.Navigation("Invoice");
                });

            modelBuilder.Entity("NovaStay.Infrastructure.Models.MaintenanceTicket", b =>
                {
                    b.HasOne("NovaStay.Infrastructure.Models.AssetAssignment", "AssetAssignment")
                        .WithMany("MaintenanceTickets")
                        .HasForeignKey("AssetAssignmentId")
                        .HasConstraintName("FK__Maintenan__Asset__44CA3770");

                    b.HasOne("NovaStay.Infrastructure.Models.Organization", "Organization")
                        .WithMany("MaintenanceTickets")
                        .HasForeignKey("OrganizationId")
                        .IsRequired()
                        .HasConstraintName("FK__Maintenan__Tenan__41EDCAC5");

                    b.HasOne("NovaStay.Infrastructure.Models.Resident", "Resident")
                        .WithMany("MaintenanceTickets")
                        .HasForeignKey("ResidentId")
                        .HasConstraintName("FK__Maintenan__Resid__43D61337");

                    b.HasOne("NovaStay.Infrastructure.Models.Room", "Room")
                        .WithMany("MaintenanceTickets")
                        .HasForeignKey("RoomId")
                        .IsRequired()
                        .HasConstraintName("FK__Maintenan__RoomI__42E1EEFE");

                    b.HasOne("NovaStay.Infrastructure.Models.Technician", "Technician")
                        .WithMany("MaintenanceTickets")
                        .HasForeignKey("TechnicianId")
                        .HasConstraintName("FK__Maintenan__Techn__45BE5BA9");

                    b.Navigation("AssetAssignment");

                    b.Navigation("Organization");

                    b.Navigation("Resident");

                    b.Navigation("Room");

                    b.Navigation("Technician");
                });

            modelBuilder.Entity("NovaStay.Infrastructure.Models.Organization", b =>
                {
                    b.HasOne("NovaStay.Infrastructure.Models.Account", "OwnerAccount")
                        .WithMany("OwnedOrganizations")
                        .HasForeignKey("OwnerAccountId")
                        .IsRequired()
                        .HasConstraintName("FK_Organizations_Accounts_OwnerAccountId");

                    b.HasOne("NovaStay.Infrastructure.Models.SubscriptionPackage", "Package")
                        .WithMany("Organizations")
                        .HasForeignKey("PackageId")
                        .IsRequired()
                        .HasConstraintName("FK__Organizations__Package__534D60F1");

                    b.Navigation("OwnerAccount");

                    b.Navigation("Package");
                });

            modelBuilder.Entity("NovaStay.Infrastructure.Models.PaymentReceipt", b =>
                {
                    b.HasOne("NovaStay.Infrastructure.Models.StaffUser", "CollectedByStaffUser")
                        .WithMany("CollectedPaymentReceipts")
                        .HasForeignKey("CollectedByStaffUserId")
                        .HasConstraintName("FK_PaymentReceipts_StaffUsers_CollectedByStaffUserId");

                    b.HasOne("NovaStay.Infrastructure.Models.Invoice", "Invoice")
                        .WithOne("PaymentReceipt")
                        .HasForeignKey("NovaStay.Infrastructure.Models.PaymentReceipt", "InvoiceId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("FK_PaymentReceipts_Invoices_InvoiceId");

                    b.HasOne("NovaStay.Infrastructure.Models.Organization", "Organization")
                        .WithMany("PaymentReceipts")
                        .HasForeignKey("OrganizationId")
                        .IsRequired()
                        .HasConstraintName("FK_PaymentReceipts_Organizations_OrganizationId");

                    b.HasOne("NovaStay.Infrastructure.Models.Property", "Property")
                        .WithMany("PaymentReceipts")
                        .HasForeignKey("PropertyId")
                        .HasConstraintName("FK_PaymentReceipts_Properties_PropertyId");

                    b.HasOne("NovaStay.Infrastructure.Models.Resident", "Resident")
                        .WithMany("PaymentReceipts")
                        .HasForeignKey("ResidentId")
                        .HasConstraintName("FK_PaymentReceipts_Residents_ResidentId");

                    b.Navigation("CollectedByStaffUser");

                    b.Navigation("Invoice");

                    b.Navigation("Organization");

                    b.Navigation("Property");

                    b.Navigation("Resident");
                });

            modelBuilder.Entity("NovaStay.Infrastructure.Models.Property", b =>
                {
                    b.HasOne("NovaStay.Infrastructure.Models.Organization", "Organization")
                        .WithMany("Properties")
                        .HasForeignKey("OrganizationId")
                        .IsRequired()
                        .HasConstraintName("FK__Propertie__Tenan__6A30C649");

                    b.Navigation("Organization");
                });

            modelBuilder.Entity("NovaStay.Infrastructure.Models.PropertyService", b =>
                {
                    b.HasOne("NovaStay.Infrastructure.Models.Property", "Property")
                        .WithMany("PropertyServices")
                        .HasForeignKey("PropertyId")
                        .IsRequired()
                        .HasConstraintName("FK_PropertyServices_Properties_PropertyId");

                    b.Navigation("Property");
                });

            modelBuilder.Entity("NovaStay.Infrastructure.Models.Resident", b =>
                {
                    b.HasOne("NovaStay.Infrastructure.Models.Account", "Account")
                        .WithMany("Residents")
                        .HasForeignKey("AccountId")
                        .IsRequired()
                        .HasConstraintName("FK_Residents_Accounts_AccountId");

                    b.Navigation("Account");
                });

            modelBuilder.Entity("NovaStay.Infrastructure.Models.ResidentMembership", b =>
                {
                    b.HasOne("NovaStay.Infrastructure.Models.Account", "Account")
                        .WithMany("ResidentMemberships")
                        .HasForeignKey("AccountId")
                        .IsRequired()
                        .HasConstraintName("FK_ResidentMemberships_Accounts_AccountId");

                    b.HasOne("NovaStay.Infrastructure.Models.Organization", "Organization")
                        .WithMany("ResidentMemberships")
                        .HasForeignKey("OrganizationId")
                        .IsRequired()
                        .HasConstraintName("FK_ResidentMemberships_Organizations_OrganizationId");

                    b.HasOne("NovaStay.Infrastructure.Models.Resident", "Resident")
                        .WithMany("ResidentMemberships")
                        .HasForeignKey("ResidentId")
                        .IsRequired()
                        .HasConstraintName("FK_ResidentMemberships_Residents_ResidentId");

                    b.Navigation("Account");

                    b.Navigation("Organization");

                    b.Navigation("Resident");
                });

            modelBuilder.Entity("NovaStay.Infrastructure.Models.Role", b =>
                {
                    b.HasOne("NovaStay.Infrastructure.Models.Organization", "Organization")
                        .WithMany("Roles")
                        .HasForeignKey("OrganizationId")
                        .IsRequired()
                        .HasConstraintName("FK__Roles__OrganizationId__5BE2A6F2");

                    b.Navigation("Organization");
                });

            modelBuilder.Entity("NovaStay.Infrastructure.Models.Room", b =>
                {
                    b.HasOne("NovaStay.Infrastructure.Models.Property", "Property")
                        .WithMany("Rooms")
                        .HasForeignKey("PropertyId")
                        .IsRequired()
                        .HasConstraintName("FK__Rooms__PropertyI__72C60C4A");

                    b.Navigation("Property");
                });

            modelBuilder.Entity("NovaStay.Infrastructure.Models.RoomAvailability", b =>
                {
                    b.HasOne("NovaStay.Infrastructure.Models.Room", "Room")
                        .WithMany("RoomAvailabilities")
                        .HasForeignKey("RoomId")
                        .IsRequired()
                        .HasConstraintName("FK__RoomAvail__RoomI__7E37BEF6");

                    b.Navigation("Room");
                });

            modelBuilder.Entity("NovaStay.Infrastructure.Models.RoomImage", b =>
                {
                    b.HasOne("NovaStay.Infrastructure.Models.Room", "Room")
                        .WithMany("RoomImages")
                        .HasForeignKey("RoomId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("FK__RoomImage__RoomI__778AC167");

                    b.Navigation("Room");
                });

            modelBuilder.Entity("NovaStay.Infrastructure.Models.RoomService", b =>
                {
                    b.HasOne("NovaStay.Infrastructure.Models.PropertyService", "PropertyService")
                        .WithMany("RoomServices")
                        .HasForeignKey("PropertyServiceId")
                        .IsRequired()
                        .HasConstraintName("FK_RoomServices_PropertyServices_PropertyServiceId");

                    b.HasOne("NovaStay.Infrastructure.Models.Room", "Room")
                        .WithMany("RoomServices")
                        .HasForeignKey("RoomId")
                        .IsRequired()
                        .HasConstraintName("FK_RoomServices_Rooms_RoomId");

                    b.Navigation("PropertyService");

                    b.Navigation("Room");
                });

            modelBuilder.Entity("NovaStay.Infrastructure.Models.StaffUser", b =>
                {
                    b.HasOne("NovaStay.Infrastructure.Models.Account", "Account")
                        .WithMany("StaffUsers")
                        .HasForeignKey("AccountId")
                        .IsRequired()
                        .HasConstraintName("FK_StaffUsers_Accounts_AccountId");

                    b.HasOne("NovaStay.Infrastructure.Models.Organization", "Organization")
                        .WithMany("StaffUsers")
                        .HasForeignKey("OrganizationId")
                        .IsRequired()
                        .HasConstraintName("FK__Users__OrganizationId__5812160E");

                    b.Navigation("Account");

                    b.Navigation("Organization");
                });

            modelBuilder.Entity("NovaStay.Infrastructure.Models.Technician", b =>
                {
                    b.HasOne("NovaStay.Infrastructure.Models.Organization", "Organization")
                        .WithMany("Technicians")
                        .HasForeignKey("OrganizationId")
                        .IsRequired()
                        .HasConstraintName("FK__Technicia__Tenan__3D2915A8");

                    b.Navigation("Organization");
                });

            modelBuilder.Entity("NovaStay.Infrastructure.Models.UtilityMeter", b =>
                {
                    b.HasOne("NovaStay.Infrastructure.Models.PropertyService", "PropertyService")
                        .WithMany("UtilityMeters")
                        .HasForeignKey("PropertyServiceId")
                        .IsRequired()
                        .HasConstraintName("FK_UtilityMeters_PropertyServices_PropertyServiceId");

                    b.HasOne("NovaStay.Infrastructure.Models.Room", "Room")
                        .WithMany("UtilityMeters")
                        .HasForeignKey("RoomId")
                        .IsRequired()
                        .HasConstraintName("FK_UtilityMeters_Rooms_RoomId");

                    b.Navigation("PropertyService");

                    b.Navigation("Room");
                });

            modelBuilder.Entity("NovaStay.Infrastructure.Models.UtilityReading", b =>
                {
                    b.HasOne("NovaStay.Infrastructure.Models.StaffUser", "RecordedByStaffUser")
                        .WithMany("UtilityReadings")
                        .HasForeignKey("RecordedByStaffUserId")
                        .HasConstraintName("FK_UtilityReadings_StaffUsers_RecordedByStaffUserId");

                    b.HasOne("NovaStay.Infrastructure.Models.UtilityMeter", "UtilityMeter")
                        .WithMany("UtilityReadings")
                        .HasForeignKey("UtilityMeterId")
                        .IsRequired()
                        .HasConstraintName("FK_UtilityReadings_UtilityMeters_UtilityMeterId");

                    b.Navigation("RecordedByStaffUser");

                    b.Navigation("UtilityMeter");
                });

            modelBuilder.Entity("NovaStay.Infrastructure.Models.UtilityTariff", b =>
                {
                    b.HasOne("NovaStay.Infrastructure.Models.PropertyService", "PropertyService")
                        .WithMany("UtilityTariffs")
                        .HasForeignKey("PropertyServiceId")
                        .IsRequired()
                        .HasConstraintName("FK_UtilityTariffs_PropertyServices_PropertyServiceId");

                    b.Navigation("PropertyService");
                });

            modelBuilder.Entity("RolePermission", b =>
                {
                    b.HasOne("NovaStay.Infrastructure.Models.Permission", null)
                        .WithMany()
                        .HasForeignKey("PermissionId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("FK__RolePermi__Permi__628FA481");

                    b.HasOne("NovaStay.Infrastructure.Models.Role", null)
                        .WithMany()
                        .HasForeignKey("RoleId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("FK__RolePermi__RoleI__619B8048");
                });

            modelBuilder.Entity("StaffUserPropertyMapping", b =>
                {
                    b.HasOne("NovaStay.Infrastructure.Models.Property", null)
                        .WithMany()
                        .HasForeignKey("PropertyId")
                        .IsRequired()
                        .HasConstraintName("FK__UserPrope__Prope__6E01572D");

                    b.HasOne("NovaStay.Infrastructure.Models.StaffUser", null)
                        .WithMany()
                        .HasForeignKey("StaffUserId")
                        .IsRequired()
                        .HasConstraintName("FK__UserPrope__UserI__6D0D32F4");
                });

            modelBuilder.Entity("StaffUserRole", b =>
                {
                    b.HasOne("NovaStay.Infrastructure.Models.Role", null)
                        .WithMany()
                        .HasForeignKey("RoleId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("FK__StaffUserRoles__RoleI__66603565");

                    b.HasOne("NovaStay.Infrastructure.Models.StaffUser", null)
                        .WithMany()
                        .HasForeignKey("StaffUserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("FK__StaffUserRoles__UserI__656C112C");
                });

            modelBuilder.Entity("NovaStay.Infrastructure.Models.Account", b =>
                {
                    b.Navigation("AccountRefreshTokens");

                    b.Navigation("OwnedOrganizations");

                    b.Navigation("ResidentMemberships");

                    b.Navigation("Residents");

                    b.Navigation("StaffUsers");
                });

            modelBuilder.Entity("NovaStay.Infrastructure.Models.Asset", b =>
                {
                    b.Navigation("AssetAssignments");
                });

            modelBuilder.Entity("NovaStay.Infrastructure.Models.AssetAssignment", b =>
                {
                    b.Navigation("MaintenanceTickets");
                });

            modelBuilder.Entity("NovaStay.Infrastructure.Models.Booking", b =>
                {
                    b.Navigation("Contracts");

                    b.Navigation("Invoices");
                });

            modelBuilder.Entity("NovaStay.Infrastructure.Models.Broker", b =>
                {
                    b.Navigation("Contracts");

                    b.Navigation("Expenses");
                });

            modelBuilder.Entity("NovaStay.Infrastructure.Models.Contract", b =>
                {
                    b.Navigation("Invoices");
                });

            modelBuilder.Entity("NovaStay.Infrastructure.Models.ExpenseCategory", b =>
                {
                    b.Navigation("Expenses");
                });

            modelBuilder.Entity("NovaStay.Infrastructure.Models.IncomeCategory", b =>
                {
                    b.Navigation("IncomeReceipts");
                });

            modelBuilder.Entity("NovaStay.Infrastructure.Models.Invoice", b =>
                {
                    b.Navigation("InvoiceLines");

                    b.Navigation("PaymentReceipt");
                });

            modelBuilder.Entity("NovaStay.Infrastructure.Models.MaintenanceTicket", b =>
                {
                    b.Navigation("Expenses");
                });

            modelBuilder.Entity("NovaStay.Infrastructure.Models.Organization", b =>
                {
                    b.Navigation("AssetAssignments");

                    b.Navigation("Assets");

                    b.Navigation("Bookings");

                    b.Navigation("Brokers");

                    b.Navigation("Contracts");

                    b.Navigation("ExpenseCategories");

                    b.Navigation("Expenses");

                    b.Navigation("IncomeCategories");

                    b.Navigation("IncomeReceipts");

                    b.Navigation("InvoiceGenerationSchedules");

                    b.Navigation("Invoices");

                    b.Navigation("MaintenanceTickets");

                    b.Navigation("PaymentReceipts");

                    b.Navigation("Properties");

                    b.Navigation("ResidentMemberships");

                    b.Navigation("Roles");

                    b.Navigation("StaffUsers");

                    b.Navigation("Technicians");
                });

            modelBuilder.Entity("NovaStay.Infrastructure.Models.Property", b =>
                {
                    b.Navigation("Bookings");

                    b.Navigation("Contracts");

                    b.Navigation("Expenses");

                    b.Navigation("IncomeReceipts");

                    b.Navigation("InvoiceGenerationSchedules");

                    b.Navigation("Invoices");

                    b.Navigation("PaymentReceipts");

                    b.Navigation("PropertyServices");

                    b.Navigation("Rooms");
                });

            modelBuilder.Entity("NovaStay.Infrastructure.Models.PropertyService", b =>
                {
                    b.Navigation("RoomServices");

                    b.Navigation("UtilityMeters");

                    b.Navigation("UtilityTariffs");
                });

            modelBuilder.Entity("NovaStay.Infrastructure.Models.Resident", b =>
                {
                    b.Navigation("Contracts");

                    b.Navigation("IncomeReceipts");

                    b.Navigation("Invoices");

                    b.Navigation("MaintenanceTickets");

                    b.Navigation("PaymentReceipts");

                    b.Navigation("ResidentMemberships");
                });

            modelBuilder.Entity("NovaStay.Infrastructure.Models.Room", b =>
                {
                    b.Navigation("AssetAssignments");

                    b.Navigation("Bookings");

                    b.Navigation("Contracts");

                    b.Navigation("Expenses");

                    b.Navigation("Invoices");

                    b.Navigation("MaintenanceTickets");

                    b.Navigation("RoomAvailabilities");

                    b.Navigation("RoomImages");

                    b.Navigation("RoomServices");

                    b.Navigation("UtilityMeters");
                });

            modelBuilder.Entity("NovaStay.Infrastructure.Models.StaffUser", b =>
                {
                    b.Navigation("ApprovedExpenses");

                    b.Navigation("CollectedIncomeReceipts");

                    b.Navigation("CollectedPaymentReceipts");

                    b.Navigation("CreatedExpenses");

                    b.Navigation("InvoiceGenerationSchedules");

                    b.Navigation("UtilityReadings");
                });

            modelBuilder.Entity("NovaStay.Infrastructure.Models.SubscriptionPackage", b =>
                {
                    b.Navigation("Organizations");
                });

            modelBuilder.Entity("NovaStay.Infrastructure.Models.Technician", b =>
                {
                    b.Navigation("MaintenanceTickets");
                });

            modelBuilder.Entity("NovaStay.Infrastructure.Models.UtilityMeter", b =>
                {
                    b.Navigation("UtilityReadings");
                });
#pragma warning restore 612, 618
        }
    }
}
