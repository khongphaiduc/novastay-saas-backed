// <auto-generated />
using System;
using Host.Infrastructure.ContextDB;
using Microsoft.EntityFrameworkCore;
using Microsoft.EntityFrameworkCore.Infrastructure;
using Microsoft.EntityFrameworkCore.Metadata;
using Microsoft.EntityFrameworkCore.Migrations;
using Microsoft.EntityFrameworkCore.Storage.ValueConversion;

#nullable disable

namespace Host.Infrastructure.Migrations
{
    [DbContext(typeof(HostContext))]
    [Migration("20260613084738_AddUserTokenTables")]
    partial class AddUserTokenTables
    {
        /// <inheritdoc />
        protected override void BuildTargetModel(ModelBuilder modelBuilder)
        {
#pragma warning disable 612, 618
            modelBuilder
                .HasAnnotation("ProductVersion", "8.0.28")
                .HasAnnotation("Relational:MaxIdentifierLength", 128);

            SqlServerModelBuilderExtensions.UseIdentityColumns(modelBuilder);

            modelBuilder.Entity("Host.Infrastructure.Models.Asset", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier")
                        .HasDefaultValueSql("(newsequentialid())");

                    b.Property<string>("AssetCode")
                        .HasMaxLength(50)
                        .IsUnicode(false)
                        .HasColumnType("varchar(50)");

                    b.Property<string>("AssetName")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<decimal?>("BaseValue")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("decimal(18, 2)")
                        .HasDefaultValue(0m);

                    b.Property<string>("Brand")
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<DateTime?>("CreatedAt")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("datetime")
                        .HasDefaultValueSql("(getdate())");

                    b.Property<string>("Model")
                        .HasMaxLength(50)
                        .IsUnicode(false)
                        .HasColumnType("varchar(50)");

                    b.Property<DateOnly?>("PurchaseDate")
                        .HasColumnType("date");

                    b.Property<Guid>("TenantId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateOnly?>("WarrantyExpiryDate")
                        .HasColumnType("date");

                    b.HasKey("Id")
                        .HasName("PK__Assets__3214EC071B1A8B14");

                    b.HasIndex("TenantId");

                    b.ToTable("Assets");
                });

            modelBuilder.Entity("Host.Infrastructure.Models.AssetAssignment", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier")
                        .HasDefaultValueSql("(newsequentialid())");

                    b.Property<Guid>("AssetId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTime?>("AssignedAt")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("datetime")
                        .HasDefaultValueSql("(getdate())");

                    b.Property<Guid?>("BedId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<string>("Note")
                        .HasMaxLength(255)
                        .HasColumnType("nvarchar(255)");

                    b.Property<Guid?>("RoomId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<string>("Status")
                        .ValueGeneratedOnAdd()
                        .HasMaxLength(20)
                        .IsUnicode(false)
                        .HasColumnType("varchar(20)")
                        .HasDefaultValue("Good");

                    b.Property<Guid>("TenantId")
                        .HasColumnType("uniqueidentifier");

                    b.HasKey("Id")
                        .HasName("PK__AssetAss__3214EC073B842B58");

                    b.HasIndex("AssetId");

                    b.HasIndex("BedId");

                    b.HasIndex("TenantId");

                    b.HasIndex(new[] { "RoomId", "BedId" }, "IX_AssetAssignments_RoomId_BedId");

                    b.ToTable("AssetAssignments");
                });

            modelBuilder.Entity("Host.Infrastructure.Models.Bed", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier")
                        .HasDefaultValueSql("(newsequentialid())");

                    b.Property<decimal>("BasePrice")
                        .HasColumnType("decimal(18, 2)");

                    b.Property<string>("BedNumber")
                        .IsRequired()
                        .HasMaxLength(20)
                        .IsUnicode(false)
                        .HasColumnType("varchar(20)");

                    b.Property<string>("CardToken")
                        .HasMaxLength(100)
                        .IsUnicode(false)
                        .HasColumnType("varchar(100)");

                    b.Property<string>("LockerId")
                        .HasMaxLength(50)
                        .IsUnicode(false)
                        .HasColumnType("varchar(50)");

                    b.Property<Guid>("RoomId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<string>("Status")
                        .IsRequired()
                        .HasMaxLength(20)
                        .IsUnicode(false)
                        .HasColumnType("varchar(20)");

                    b.HasKey("Id")
                        .HasName("PK__Beds__3214EC07D3D03E54");

                    b.HasIndex("RoomId");

                    b.ToTable("Beds");
                });

            modelBuilder.Entity("Host.Infrastructure.Models.Booking", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier")
                        .HasDefaultValueSql("(newsequentialid())");

                    b.Property<decimal>("Amount")
                        .HasColumnType("decimal(18, 2)");

                    b.Property<Guid?>("BedId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<string>("BookingType")
                        .IsRequired()
                        .HasMaxLength(20)
                        .IsUnicode(false)
                        .HasColumnType("varchar(20)");

                    b.Property<DateOnly>("CheckInDate")
                        .HasColumnType("date");

                    b.Property<DateOnly?>("CheckOutDate")
                        .HasColumnType("date");

                    b.Property<DateTime?>("CreatedAt")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("datetime")
                        .HasDefaultValueSql("(getdate())");

                    b.Property<string>("GuestEmail")
                        .HasMaxLength(100)
                        .IsUnicode(false)
                        .HasColumnType("varchar(100)");

                    b.Property<string>("GuestName")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<string>("GuestPhone")
                        .IsRequired()
                        .HasMaxLength(15)
                        .IsUnicode(false)
                        .HasColumnType("varchar(15)");

                    b.Property<string>("PaymentStatus")
                        .ValueGeneratedOnAdd()
                        .HasMaxLength(20)
                        .IsUnicode(false)
                        .HasColumnType("varchar(20)")
                        .HasDefaultValue("Pending");

                    b.Property<string>("PaymentTransactionId")
                        .HasMaxLength(100)
                        .IsUnicode(false)
                        .HasColumnType("varchar(100)");

                    b.Property<Guid>("PropertyId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<Guid>("RoomId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<Guid>("TenantId")
                        .HasColumnType("uniqueidentifier");

                    b.HasKey("Id")
                        .HasName("PK__Bookings__3214EC07E0378A72");

                    b.HasIndex("BedId");

                    b.HasIndex("PropertyId");

                    b.HasIndex("RoomId");

                    b.HasIndex("TenantId");

                    b.ToTable("Bookings");
                });

            modelBuilder.Entity("Host.Infrastructure.Models.Broker", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier")
                        .HasDefaultValueSql("(newsequentialid())");

                    b.Property<string>("FullName")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<string>("Phone")
                        .IsRequired()
                        .HasMaxLength(15)
                        .IsUnicode(false)
                        .HasColumnType("varchar(15)");

                    b.Property<Guid>("TenantId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<decimal?>("TotalCommissionEarned")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("decimal(18, 2)")
                        .HasDefaultValue(0m);

                    b.Property<decimal?>("WalletBalance")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("decimal(18, 2)")
                        .HasDefaultValue(0m);

                    b.HasKey("Id")
                        .HasName("PK__Brokers__3214EC07D3AA9FC2");

                    b.HasIndex("TenantId");

                    b.ToTable("Brokers");
                });

            modelBuilder.Entity("Host.Infrastructure.Models.Contract", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier")
                        .HasDefaultValueSql("(newsequentialid())");

                    b.Property<Guid?>("BedId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<Guid?>("BookingId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<decimal?>("BrokerCommission")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("decimal(18, 2)")
                        .HasDefaultValue(0m);

                    b.Property<Guid?>("BrokerId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<string>("CommissionStatus")
                        .ValueGeneratedOnAdd()
                        .HasMaxLength(20)
                        .IsUnicode(false)
                        .HasColumnType("varchar(20)")
                        .HasDefaultValue("Pending");

                    b.Property<string>("ContractPdfUrl")
                        .HasMaxLength(255)
                        .IsUnicode(false)
                        .HasColumnType("varchar(255)");

                    b.Property<DateTime?>("CreatedAt")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("datetime")
                        .HasDefaultValueSql("(getdate())");

                    b.Property<decimal>("DepositAmount")
                        .HasColumnType("decimal(18, 2)");

                    b.Property<DateOnly>("EndDate")
                        .HasColumnType("date");

                    b.Property<Guid>("PropertyId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<Guid>("ResidentId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<Guid>("RoomId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateOnly>("StartDate")
                        .HasColumnType("date");

                    b.Property<string>("Status")
                        .IsRequired()
                        .HasMaxLength(20)
                        .IsUnicode(false)
                        .HasColumnType("varchar(20)");

                    b.Property<Guid>("TenantId")
                        .HasColumnType("uniqueidentifier");

                    b.HasKey("Id")
                        .HasName("PK__Contract__3214EC0707402DA0");

                    b.HasIndex("BedId");

                    b.HasIndex("BookingId");

                    b.HasIndex("BrokerId");

                    b.HasIndex("PropertyId");

                    b.HasIndex("ResidentId");

                    b.HasIndex("RoomId");

                    b.HasIndex("TenantId");

                    b.ToTable("Contracts");
                });

            modelBuilder.Entity("Host.Infrastructure.Models.Invoice", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier")
                        .HasDefaultValueSql("(newsequentialid())");

                    b.Property<Guid?>("BookingId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<Guid?>("ContractId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTime?>("CreatedAt")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("datetime")
                        .HasDefaultValueSql("(getdate())");

                    b.Property<string>("InvoicePeriod")
                        .IsRequired()
                        .HasMaxLength(7)
                        .IsUnicode(false)
                        .HasColumnType("varchar(7)");

                    b.Property<DateTime?>("PaidAt")
                        .HasColumnType("datetime");

                    b.Property<string>("QrCodeUrl")
                        .HasMaxLength(500)
                        .IsUnicode(false)
                        .HasColumnType("varchar(500)");

                    b.Property<decimal>("RoomPrice")
                        .HasColumnType("decimal(18, 2)");

                    b.Property<decimal>("ServicesPrice")
                        .HasColumnType("decimal(18, 2)");

                    b.Property<string>("Status")
                        .ValueGeneratedOnAdd()
                        .HasMaxLength(20)
                        .IsUnicode(false)
                        .HasColumnType("varchar(20)")
                        .HasDefaultValue("Unpaid");

                    b.Property<Guid>("TenantId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<decimal>("TotalAmount")
                        .HasColumnType("decimal(18, 2)");

                    b.HasKey("Id")
                        .HasName("PK__Invoices__3214EC07023BFFC1");

                    b.HasIndex("BookingId");

                    b.HasIndex("ContractId");

                    b.HasIndex("TenantId");

                    b.HasIndex(new[] { "Status", "InvoicePeriod" }, "IX_Invoices_Status_Period");

                    b.ToTable("Invoices");
                });

            modelBuilder.Entity("Host.Infrastructure.Models.Listing", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier")
                        .HasDefaultValueSql("(newsequentialid())");

                    b.Property<string>("Amenities")
                        .HasMaxLength(500)
                        .HasColumnType("nvarchar(500)");

                    b.Property<Guid?>("BedId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTime?>("CreatedAt")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("datetime")
                        .HasDefaultValueSql("(getdate())");

                    b.Property<string>("Description")
                        .HasColumnType("nvarchar(max)");

                    b.Property<bool?>("IsPublished")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bit")
                        .HasDefaultValue(true);

                    b.Property<Guid>("PropertyId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<Guid>("RoomId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<Guid>("TenantId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<string>("Title")
                        .IsRequired()
                        .HasMaxLength(200)
                        .HasColumnType("nvarchar(200)");

                    b.HasKey("Id")
                        .HasName("PK__Listings__3214EC0739EB0492");

                    b.HasIndex("BedId");

                    b.HasIndex("PropertyId");

                    b.HasIndex("RoomId");

                    b.HasIndex("TenantId");

                    b.HasIndex(new[] { "IsPublished" }, "IX_Listings_IsPublished");

                    b.ToTable("Listings");
                });

            modelBuilder.Entity("Host.Infrastructure.Models.ListingImage", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier")
                        .HasDefaultValueSql("(newsequentialid())");

                    b.Property<int?>("DisplayOrder")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasDefaultValue(0);

                    b.Property<string>("ImageUrl")
                        .IsRequired()
                        .HasMaxLength(255)
                        .IsUnicode(false)
                        .HasColumnType("varchar(255)");

                    b.Property<Guid>("ListingId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTime?>("UploadedAt")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("datetime")
                        .HasDefaultValueSql("(getdate())");

                    b.HasKey("Id")
                        .HasName("PK__ListingI__3214EC072BEE08C6");

                    b.HasIndex(new[] { "ListingId" }, "IX_ListingImages_ListingId");

                    b.ToTable("ListingImages");
                });

            modelBuilder.Entity("Host.Infrastructure.Models.MaintenanceTicket", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier")
                        .HasDefaultValueSql("(newsequentialid())");

                    b.Property<Guid?>("AssetAssignmentId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<string>("Category")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<DateTime?>("CreatedAt")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("datetime")
                        .HasDefaultValueSql("(getdate())");

                    b.Property<string>("IncidentImageUrl")
                        .HasMaxLength(255)
                        .IsUnicode(false)
                        .HasColumnType("varchar(255)");

                    b.Property<Guid>("ResidentId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<string>("ResolvedImageUrl")
                        .HasMaxLength(255)
                        .IsUnicode(false)
                        .HasColumnType("varchar(255)");

                    b.Property<Guid>("RoomId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<string>("Status")
                        .ValueGeneratedOnAdd()
                        .HasMaxLength(20)
                        .IsUnicode(false)
                        .HasColumnType("varchar(20)")
                        .HasDefaultValue("Pending");

                    b.Property<Guid?>("TechnicianId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<Guid>("TenantId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTime?>("UpdatedAt")
                        .HasColumnType("datetime");

                    b.Property<string>("UserDescription")
                        .IsRequired()
                        .HasMaxLength(500)
                        .HasColumnType("nvarchar(500)");

                    b.HasKey("Id")
                        .HasName("PK__Maintena__3214EC071D5966C6");

                    b.HasIndex("AssetAssignmentId");

                    b.HasIndex("ResidentId");

                    b.HasIndex("RoomId");

                    b.HasIndex("TechnicianId");

                    b.HasIndex("TenantId");

                    b.ToTable("MaintenanceTickets");
                });

            modelBuilder.Entity("Host.Infrastructure.Models.Permission", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier")
                        .HasDefaultValueSql("(newsequentialid())");

                    b.Property<string>("Module")
                        .IsRequired()
                        .HasMaxLength(50)
                        .IsUnicode(false)
                        .HasColumnType("varchar(50)");

                    b.Property<string>("PermissionKey")
                        .IsRequired()
                        .HasMaxLength(100)
                        .IsUnicode(false)
                        .HasColumnType("varchar(100)");

                    b.Property<string>("PermissionName")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.HasKey("Id")
                        .HasName("PK__Permissi__3214EC0720D981B2");

                    b.HasIndex(new[] { "PermissionKey" }, "UQ__Permissi__8884ABD46702546C")
                        .IsUnique();

                    b.ToTable("Permissions");
                });

            modelBuilder.Entity("Host.Infrastructure.Models.Property", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier")
                        .HasDefaultValueSql("(newsequentialid())");

                    b.Property<string>("Address")
                        .IsRequired()
                        .HasMaxLength(255)
                        .HasColumnType("nvarchar(255)");

                    b.Property<DateTime?>("CreatedAt")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("datetime")
                        .HasDefaultValueSql("(getdate())");

                    b.Property<string>("PropertyName")
                        .IsRequired()
                        .HasMaxLength(150)
                        .HasColumnType("nvarchar(150)");

                    b.Property<string>("PropertyType")
                        .IsRequired()
                        .HasMaxLength(20)
                        .IsUnicode(false)
                        .HasColumnType("varchar(20)");

                    b.Property<Guid>("TenantId")
                        .HasColumnType("uniqueidentifier");

                    b.HasKey("Id")
                        .HasName("PK__Properti__3214EC07FE441D45");

                    b.HasIndex("TenantId");

                    b.ToTable("Properties");
                });

            modelBuilder.Entity("Host.Infrastructure.Models.Resident", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier")
                        .HasDefaultValueSql("(newsequentialid())");

                    b.Property<DateTime?>("CreatedAt")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("datetime")
                        .HasDefaultValueSql("(getdate())");

                    b.Property<string>("Email")
                        .HasMaxLength(100)
                        .IsUnicode(false)
                        .HasColumnType("varchar(100)");

                    b.Property<string>("FullName")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<string>("IdBackImageUrl")
                        .HasMaxLength(255)
                        .IsUnicode(false)
                        .HasColumnType("varchar(255)");

                    b.Property<string>("IdFrontImageUrl")
                        .HasMaxLength(255)
                        .IsUnicode(false)
                        .HasColumnType("varchar(255)");

                    b.Property<string>("IdentityCardNumber")
                        .HasMaxLength(20)
                        .IsUnicode(false)
                        .HasColumnType("varchar(20)");

                    b.Property<string>("Phone")
                        .IsRequired()
                        .HasMaxLength(15)
                        .IsUnicode(false)
                        .HasColumnType("varchar(15)");

                    b.Property<string>("ProfileImageUrl")
                        .HasMaxLength(255)
                        .IsUnicode(false)
                        .HasColumnType("varchar(255)");

                    b.Property<Guid>("TenantId")
                        .HasColumnType("uniqueidentifier");

                    b.HasKey("Id")
                        .HasName("PK__Resident__3214EC07115F9503");

                    b.HasIndex("TenantId");

                    b.ToTable("Residents");
                });

            modelBuilder.Entity("Host.Infrastructure.Models.Role", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier")
                        .HasDefaultValueSql("(newsequentialid())");

                    b.Property<DateTime?>("CreatedAt")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("datetime")
                        .HasDefaultValueSql("(getdate())");

                    b.Property<string>("Description")
                        .HasMaxLength(255)
                        .HasColumnType("nvarchar(255)");

                    b.Property<string>("RoleKey")
                        .IsRequired()
                        .HasMaxLength(50)
                        .IsUnicode(false)
                        .HasColumnType("varchar(50)");

                    b.Property<string>("RoleName")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<Guid>("TenantId")
                        .HasColumnType("uniqueidentifier");

                    b.HasKey("Id")
                        .HasName("PK__Roles__3214EC07BE72DF6E");

                    b.HasIndex("TenantId");

                    b.ToTable("Roles");
                });

            modelBuilder.Entity("Host.Infrastructure.Models.Room", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier")
                        .HasDefaultValueSql("(newsequentialid())");

                    b.Property<decimal>("BasePrice")
                        .HasColumnType("decimal(18, 2)");

                    b.Property<DateTime?>("CreatedAt")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("datetime")
                        .HasDefaultValueSql("(getdate())");

                    b.Property<int>("Floor")
                        .HasColumnType("int");

                    b.Property<int?>("MaxOccupants")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasDefaultValue(1);

                    b.Property<Guid>("PropertyId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<string>("RoomNumber")
                        .IsRequired()
                        .HasMaxLength(20)
                        .IsUnicode(false)
                        .HasColumnType("varchar(20)");

                    b.Property<byte[]>("RowVersion")
                        .IsConcurrencyToken()
                        .IsRequired()
                        .ValueGeneratedOnAddOrUpdate()
                        .HasColumnType("rowversion");

                    b.Property<string>("Status")
                        .IsRequired()
                        .HasMaxLength(20)
                        .IsUnicode(false)
                        .HasColumnType("varchar(20)");

                    b.HasKey("Id")
                        .HasName("PK__Rooms__3214EC077FA99C97");

                    b.HasIndex(new[] { "PropertyId", "Status" }, "IX_Rooms_PropertyId_Status");

                    b.ToTable("Rooms");
                });

            modelBuilder.Entity("Host.Infrastructure.Models.RoomAvailability", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier")
                        .HasDefaultValueSql("(newsequentialid())");

                    b.Property<decimal>("DynamicPrice")
                        .HasColumnType("decimal(18, 2)");

                    b.Property<bool?>("IsBooked")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bit")
                        .HasDefaultValue(false);

                    b.Property<Guid>("RoomId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateOnly>("StayDate")
                        .HasColumnType("date");

                    b.HasKey("Id")
                        .HasName("PK__RoomAvai__3214EC07207FCE01");

                    b.HasIndex("RoomId");

                    b.ToTable("RoomAvailabilities");
                });

            modelBuilder.Entity("Host.Infrastructure.Models.RoomImage", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier")
                        .HasDefaultValueSql("(newsequentialid())");

                    b.Property<string>("ImageUrl")
                        .IsRequired()
                        .HasMaxLength(255)
                        .IsUnicode(false)
                        .HasColumnType("varchar(255)");

                    b.Property<bool?>("IsCover")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bit")
                        .HasDefaultValue(false);

                    b.Property<Guid>("RoomId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<DateTime?>("UploadedAt")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("datetime")
                        .HasDefaultValueSql("(getdate())");

                    b.HasKey("Id")
                        .HasName("PK__RoomImag__3214EC079F9973C0");

                    b.HasIndex(new[] { "RoomId" }, "IX_RoomImages_RoomId");

                    b.ToTable("RoomImages");
                });

            modelBuilder.Entity("Host.Infrastructure.Models.SubscriptionPackage", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier")
                        .HasDefaultValueSql("(newsequentialid())");

                    b.Property<bool?>("AllowAiFeatures")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bit")
                        .HasDefaultValue(false);

                    b.Property<bool?>("AllowCustomRoles")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bit")
                        .HasDefaultValue(false);

                    b.Property<DateTime?>("CreatedAt")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("datetime")
                        .HasDefaultValueSql("(getdate())");

                    b.Property<int>("MaxProperties")
                        .HasColumnType("int");

                    b.Property<int>("MaxRooms")
                        .HasColumnType("int");

                    b.Property<string>("PackageKey")
                        .IsRequired()
                        .HasMaxLength(20)
                        .IsUnicode(false)
                        .HasColumnType("varchar(20)");

                    b.Property<string>("PackageName")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<decimal>("PriceMonthly")
                        .HasColumnType("decimal(18, 2)");

                    b.Property<int?>("TokenGiftMonthly")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasDefaultValue(0);

                    b.HasKey("Id")
                        .HasName("PK__Subscrip__3214EC077D1660D7");

                    b.HasIndex(new[] { "PackageKey" }, "UQ__Subscrip__63953E72B6E7DAB9")
                        .IsUnique();

                    b.ToTable("SubscriptionPackages");
                });

            modelBuilder.Entity("Host.Infrastructure.Models.Technician", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier")
                        .HasDefaultValueSql("(newsequentialid())");

                    b.Property<string>("FullName")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<bool?>("IsAvailable")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bit")
                        .HasDefaultValue(true);

                    b.Property<string>("Phone")
                        .IsRequired()
                        .HasMaxLength(15)
                        .IsUnicode(false)
                        .HasColumnType("varchar(15)");

                    b.Property<string>("Specialty")
                        .IsRequired()
                        .HasMaxLength(50)
                        .HasColumnType("nvarchar(50)");

                    b.Property<Guid>("TenantId")
                        .HasColumnType("uniqueidentifier");

                    b.HasKey("Id")
                        .HasName("PK__Technici__3214EC0701E593D1");

                    b.HasIndex("TenantId");

                    b.ToTable("Technicians");
                });

            modelBuilder.Entity("Host.Infrastructure.Models.Tenant", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier")
                        .HasDefaultValueSql("(newsequentialid())");

                    b.Property<string>("BusinessName")
                        .IsRequired()
                        .HasMaxLength(150)
                        .HasColumnType("nvarchar(150)");

                    b.Property<DateTime?>("CreatedAt")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("datetime")
                        .HasDefaultValueSql("(getdate())");

                    b.Property<string>("OwnerEmail")
                        .IsRequired()
                        .HasMaxLength(100)
                        .IsUnicode(false)
                        .HasColumnType("varchar(100)");

                    b.Property<string>("OwnerPhone")
                        .IsRequired()
                        .HasMaxLength(15)
                        .IsUnicode(false)
                        .HasColumnType("varchar(15)");

                    b.Property<Guid>("PackageId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<string>("SubscriptionStatus")
                        .IsRequired()
                        .HasMaxLength(20)
                        .IsUnicode(false)
                        .HasColumnType("varchar(20)");

                    b.Property<string>("TaxCode")
                        .HasMaxLength(20)
                        .IsUnicode(false)
                        .HasColumnType("varchar(20)");

                    b.Property<int?>("TokenBalance")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("int")
                        .HasDefaultValue(0);

                    b.Property<DateTime?>("UpdatedAt")
                        .HasColumnType("datetime");

                    b.HasKey("Id")
                        .HasName("PK__Tenants__3214EC070623D35C");

                    b.HasIndex(new[] { "PackageId" }, "IX_Tenants_PackageId");

                    b.HasIndex(new[] { "OwnerEmail" }, "UQ__Tenants__FF0186BBAE6B6868")
                        .IsUnique();

                    b.ToTable("Tenants");
                });

            modelBuilder.Entity("Host.Infrastructure.Models.User", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier")
                        .HasDefaultValueSql("(newsequentialid())");

                    b.Property<DateTime?>("CreatedAt")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("datetime")
                        .HasDefaultValueSql("(getdate())");

                    b.Property<string>("Email")
                        .HasMaxLength(100)
                        .IsUnicode(false)
                        .HasColumnType("varchar(100)");

                    b.Property<string>("FullName")
                        .IsRequired()
                        .HasMaxLength(100)
                        .HasColumnType("nvarchar(100)");

                    b.Property<bool?>("IsActive")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("bit")
                        .HasDefaultValue(true);

                    b.Property<string>("PasswordHash")
                        .IsRequired()
                        .HasMaxLength(255)
                        .IsUnicode(false)
                        .HasColumnType("varchar(255)");

                    b.Property<string>("Phone")
                        .IsRequired()
                        .HasMaxLength(15)
                        .IsUnicode(false)
                        .HasColumnType("varchar(15)");

                    b.Property<Guid>("TenantId")
                        .HasColumnType("uniqueidentifier");

                    b.HasKey("Id")
                        .HasName("PK__Users__3214EC078909D83A");

                    b.HasIndex(new[] { "TenantId" }, "IX_Users_TenantId");

                    b.ToTable("Users");
                });

            modelBuilder.Entity("Host.Infrastructure.Models.UserAccessToken", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier")
                        .HasDefaultValueSql("(newsequentialid())");

                    b.Property<DateTime?>("CreatedAt")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("datetime")
                        .HasDefaultValueSql("(getdate())");

                    b.Property<string>("CreatedByIp")
                        .HasMaxLength(45)
                        .IsUnicode(false)
                        .HasColumnType("varchar(45)");

                    b.Property<DateTime>("ExpiresAt")
                        .HasColumnType("datetime");

                    b.Property<DateTime?>("RevokedAt")
                        .HasColumnType("datetime");

                    b.Property<string>("RevokedByIp")
                        .HasMaxLength(45)
                        .IsUnicode(false)
                        .HasColumnType("varchar(45)");

                    b.Property<string>("TokenHash")
                        .IsRequired()
                        .HasMaxLength(512)
                        .IsUnicode(false)
                        .HasColumnType("varchar(512)");

                    b.Property<Guid>("UserId")
                        .HasColumnType("uniqueidentifier");

                    b.HasKey("Id")
                        .HasName("PK_UserAccessTokens");

                    b.HasIndex(new[] { "TokenHash" }, "IX_UserAccessTokens_TokenHash");

                    b.HasIndex(new[] { "UserId", "ExpiresAt" }, "IX_UserAccessTokens_UserId_ExpiresAt");

                    b.ToTable("UserAccessTokens");
                });

            modelBuilder.Entity("Host.Infrastructure.Models.UserRefreshToken", b =>
                {
                    b.Property<Guid>("Id")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("uniqueidentifier")
                        .HasDefaultValueSql("(newsequentialid())");

                    b.Property<DateTime?>("CreatedAt")
                        .ValueGeneratedOnAdd()
                        .HasColumnType("datetime")
                        .HasDefaultValueSql("(getdate())");

                    b.Property<string>("CreatedByIp")
                        .HasMaxLength(45)
                        .IsUnicode(false)
                        .HasColumnType("varchar(45)");

                    b.Property<DateTime>("ExpiresAt")
                        .HasColumnType("datetime");

                    b.Property<string>("ReplacedByTokenHash")
                        .HasMaxLength(512)
                        .IsUnicode(false)
                        .HasColumnType("varchar(512)");

                    b.Property<DateTime?>("RevokedAt")
                        .HasColumnType("datetime");

                    b.Property<string>("RevokedByIp")
                        .HasMaxLength(45)
                        .IsUnicode(false)
                        .HasColumnType("varchar(45)");

                    b.Property<string>("TokenHash")
                        .IsRequired()
                        .HasMaxLength(512)
                        .IsUnicode(false)
                        .HasColumnType("varchar(512)");

                    b.Property<Guid>("UserId")
                        .HasColumnType("uniqueidentifier");

                    b.HasKey("Id")
                        .HasName("PK_UserRefreshTokens");

                    b.HasIndex(new[] { "TokenHash" }, "IX_UserRefreshTokens_TokenHash")
                        .IsUnique();

                    b.HasIndex(new[] { "UserId", "ExpiresAt" }, "IX_UserRefreshTokens_UserId_ExpiresAt");

                    b.ToTable("UserRefreshTokens");
                });

            modelBuilder.Entity("RolePermission", b =>
                {
                    b.Property<Guid>("RoleId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<Guid>("PermissionId")
                        .HasColumnType("uniqueidentifier");

                    b.HasKey("RoleId", "PermissionId")
                        .HasName("PK__RolePerm__6400A1A8CB2E4434");

                    b.HasIndex("PermissionId");

                    b.ToTable("RolePermissions", (string)null);
                });

            modelBuilder.Entity("UserPropertyMapping", b =>
                {
                    b.Property<Guid>("UserId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<Guid>("PropertyId")
                        .HasColumnType("uniqueidentifier");

                    b.HasKey("UserId", "PropertyId")
                        .HasName("PK__UserProp__5084563FC247FEEC");

                    b.HasIndex("PropertyId");

                    b.ToTable("UserPropertyMapping", (string)null);
                });

            modelBuilder.Entity("UserRole", b =>
                {
                    b.Property<Guid>("UserId")
                        .HasColumnType("uniqueidentifier");

                    b.Property<Guid>("RoleId")
                        .HasColumnType("uniqueidentifier");

                    b.HasKey("UserId", "RoleId")
                        .HasName("PK__UserRole__AF2760AD29238DE0");

                    b.HasIndex("RoleId");

                    b.ToTable("UserRoles", (string)null);
                });

            modelBuilder.Entity("Host.Infrastructure.Models.Asset", b =>
                {
                    b.HasOne("Host.Infrastructure.Models.Tenant", "Tenant")
                        .WithMany("Assets")
                        .HasForeignKey("TenantId")
                        .IsRequired()
                        .HasConstraintName("FK__Assets__TenantId__02FC7413");

                    b.Navigation("Tenant");
                });

            modelBuilder.Entity("Host.Infrastructure.Models.AssetAssignment", b =>
                {
                    b.HasOne("Host.Infrastructure.Models.Asset", "Asset")
                        .WithMany("AssetAssignments")
                        .HasForeignKey("AssetId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("FK__AssetAssi__Asset__08B54D69");

                    b.HasOne("Host.Infrastructure.Models.Bed", "Bed")
                        .WithMany("AssetAssignments")
                        .HasForeignKey("BedId")
                        .HasConstraintName("FK__AssetAssi__BedId__0A9D95DB");

                    b.HasOne("Host.Infrastructure.Models.Room", "Room")
                        .WithMany("AssetAssignments")
                        .HasForeignKey("RoomId")
                        .HasConstraintName("FK__AssetAssi__RoomI__09A971A2");

                    b.HasOne("Host.Infrastructure.Models.Tenant", "Tenant")
                        .WithMany("AssetAssignments")
                        .HasForeignKey("TenantId")
                        .IsRequired()
                        .HasConstraintName("FK__AssetAssi__Tenan__07C12930");

                    b.Navigation("Asset");

                    b.Navigation("Bed");

                    b.Navigation("Room");

                    b.Navigation("Tenant");
                });

            modelBuilder.Entity("Host.Infrastructure.Models.Bed", b =>
                {
                    b.HasOne("Host.Infrastructure.Models.Room", "Room")
                        .WithMany("Beds")
                        .HasForeignKey("RoomId")
                        .IsRequired()
                        .HasConstraintName("FK__Beds__RoomId__7A672E12");

                    b.Navigation("Room");
                });

            modelBuilder.Entity("Host.Infrastructure.Models.Booking", b =>
                {
                    b.HasOne("Host.Infrastructure.Models.Bed", "Bed")
                        .WithMany("Bookings")
                        .HasForeignKey("BedId")
                        .HasConstraintName("FK__Bookings__BedId__1EA48E88");

                    b.HasOne("Host.Infrastructure.Models.Property", "Property")
                        .WithMany("Bookings")
                        .HasForeignKey("PropertyId")
                        .IsRequired()
                        .HasConstraintName("FK__Bookings__Proper__1CBC4616");

                    b.HasOne("Host.Infrastructure.Models.Room", "Room")
                        .WithMany("Bookings")
                        .HasForeignKey("RoomId")
                        .IsRequired()
                        .HasConstraintName("FK__Bookings__RoomId__1DB06A4F");

                    b.HasOne("Host.Infrastructure.Models.Tenant", "Tenant")
                        .WithMany("Bookings")
                        .HasForeignKey("TenantId")
                        .IsRequired()
                        .HasConstraintName("FK__Bookings__Tenant__1BC821DD");

                    b.Navigation("Bed");

                    b.Navigation("Property");

                    b.Navigation("Room");

                    b.Navigation("Tenant");
                });

            modelBuilder.Entity("Host.Infrastructure.Models.Broker", b =>
                {
                    b.HasOne("Host.Infrastructure.Models.Tenant", "Tenant")
                        .WithMany("Brokers")
                        .HasForeignKey("TenantId")
                        .IsRequired()
                        .HasConstraintName("FK__Brokers__TenantI__2739D489");

                    b.Navigation("Tenant");
                });

            modelBuilder.Entity("Host.Infrastructure.Models.Contract", b =>
                {
                    b.HasOne("Host.Infrastructure.Models.Bed", "Bed")
                        .WithMany("Contracts")
                        .HasForeignKey("BedId")
                        .HasConstraintName("FK__Contracts__BedId__2FCF1A8A");

                    b.HasOne("Host.Infrastructure.Models.Booking", "Booking")
                        .WithMany("Contracts")
                        .HasForeignKey("BookingId")
                        .HasConstraintName("FK__Contracts__Booki__32AB8735");

                    b.HasOne("Host.Infrastructure.Models.Broker", "Broker")
                        .WithMany("Contracts")
                        .HasForeignKey("BrokerId")
                        .HasConstraintName("FK__Contracts__Broke__31B762FC");

                    b.HasOne("Host.Infrastructure.Models.Property", "Property")
                        .WithMany("Contracts")
                        .HasForeignKey("PropertyId")
                        .IsRequired()
                        .HasConstraintName("FK__Contracts__Prope__2DE6D218");

                    b.HasOne("Host.Infrastructure.Models.Resident", "Resident")
                        .WithMany("Contracts")
                        .HasForeignKey("ResidentId")
                        .IsRequired()
                        .HasConstraintName("FK__Contracts__Resid__30C33EC3");

                    b.HasOne("Host.Infrastructure.Models.Room", "Room")
                        .WithMany("Contracts")
                        .HasForeignKey("RoomId")
                        .IsRequired()
                        .HasConstraintName("FK__Contracts__RoomI__2EDAF651");

                    b.HasOne("Host.Infrastructure.Models.Tenant", "Tenant")
                        .WithMany("Contracts")
                        .HasForeignKey("TenantId")
                        .IsRequired()
                        .HasConstraintName("FK__Contracts__Tenan__2CF2ADDF");

                    b.Navigation("Bed");

                    b.Navigation("Booking");

                    b.Navigation("Broker");

                    b.Navigation("Property");

                    b.Navigation("Resident");

                    b.Navigation("Room");

                    b.Navigation("Tenant");
                });

            modelBuilder.Entity("Host.Infrastructure.Models.Invoice", b =>
                {
                    b.HasOne("Host.Infrastructure.Models.Booking", "Booking")
                        .WithMany("Invoices")
                        .HasForeignKey("BookingId")
                        .HasConstraintName("FK__Invoices__Bookin__395884C4");

                    b.HasOne("Host.Infrastructure.Models.Contract", "Contract")
                        .WithMany("Invoices")
                        .HasForeignKey("ContractId")
                        .HasConstraintName("FK__Invoices__Contra__3864608B");

                    b.HasOne("Host.Infrastructure.Models.Tenant", "Tenant")
                        .WithMany("Invoices")
                        .HasForeignKey("TenantId")
                        .IsRequired()
                        .HasConstraintName("FK__Invoices__Tenant__37703C52");

                    b.Navigation("Booking");

                    b.Navigation("Contract");

                    b.Navigation("Tenant");
                });

            modelBuilder.Entity("Host.Infrastructure.Models.Listing", b =>
                {
                    b.HasOne("Host.Infrastructure.Models.Bed", "Bed")
                        .WithMany("Listings")
                        .HasForeignKey("BedId")
                        .HasConstraintName("FK__Listings__BedId__123EB7A3");

                    b.HasOne("Host.Infrastructure.Models.Property", "Property")
                        .WithMany("Listings")
                        .HasForeignKey("PropertyId")
                        .IsRequired()
                        .HasConstraintName("FK__Listings__Proper__10566F31");

                    b.HasOne("Host.Infrastructure.Models.Room", "Room")
                        .WithMany("Listings")
                        .HasForeignKey("RoomId")
                        .IsRequired()
                        .HasConstraintName("FK__Listings__RoomId__114A936A");

                    b.HasOne("Host.Infrastructure.Models.Tenant", "Tenant")
                        .WithMany("Listings")
                        .HasForeignKey("TenantId")
                        .IsRequired()
                        .HasConstraintName("FK__Listings__Tenant__0F624AF8");

                    b.Navigation("Bed");

                    b.Navigation("Property");

                    b.Navigation("Room");

                    b.Navigation("Tenant");
                });

            modelBuilder.Entity("Host.Infrastructure.Models.ListingImage", b =>
                {
                    b.HasOne("Host.Infrastructure.Models.Listing", "Listing")
                        .WithMany("ListingImages")
                        .HasForeignKey("ListingId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("FK__ListingIm__Listi__17036CC0");

                    b.Navigation("Listing");
                });

            modelBuilder.Entity("Host.Infrastructure.Models.MaintenanceTicket", b =>
                {
                    b.HasOne("Host.Infrastructure.Models.AssetAssignment", "AssetAssignment")
                        .WithMany("MaintenanceTickets")
                        .HasForeignKey("AssetAssignmentId")
                        .HasConstraintName("FK__Maintenan__Asset__44CA3770");

                    b.HasOne("Host.Infrastructure.Models.Resident", "Resident")
                        .WithMany("MaintenanceTickets")
                        .HasForeignKey("ResidentId")
                        .IsRequired()
                        .HasConstraintName("FK__Maintenan__Resid__43D61337");

                    b.HasOne("Host.Infrastructure.Models.Room", "Room")
                        .WithMany("MaintenanceTickets")
                        .HasForeignKey("RoomId")
                        .IsRequired()
                        .HasConstraintName("FK__Maintenan__RoomI__42E1EEFE");

                    b.HasOne("Host.Infrastructure.Models.Technician", "Technician")
                        .WithMany("MaintenanceTickets")
                        .HasForeignKey("TechnicianId")
                        .HasConstraintName("FK__Maintenan__Techn__45BE5BA9");

                    b.HasOne("Host.Infrastructure.Models.Tenant", "Tenant")
                        .WithMany("MaintenanceTickets")
                        .HasForeignKey("TenantId")
                        .IsRequired()
                        .HasConstraintName("FK__Maintenan__Tenan__41EDCAC5");

                    b.Navigation("AssetAssignment");

                    b.Navigation("Resident");

                    b.Navigation("Room");

                    b.Navigation("Technician");

                    b.Navigation("Tenant");
                });

            modelBuilder.Entity("Host.Infrastructure.Models.Property", b =>
                {
                    b.HasOne("Host.Infrastructure.Models.Tenant", "Tenant")
                        .WithMany("Properties")
                        .HasForeignKey("TenantId")
                        .IsRequired()
                        .HasConstraintName("FK__Propertie__Tenan__6A30C649");

                    b.Navigation("Tenant");
                });

            modelBuilder.Entity("Host.Infrastructure.Models.Resident", b =>
                {
                    b.HasOne("Host.Infrastructure.Models.Tenant", "Tenant")
                        .WithMany("Residents")
                        .HasForeignKey("TenantId")
                        .IsRequired()
                        .HasConstraintName("FK__Residents__Tenan__22751F6C");

                    b.Navigation("Tenant");
                });

            modelBuilder.Entity("Host.Infrastructure.Models.Role", b =>
                {
                    b.HasOne("Host.Infrastructure.Models.Tenant", "Tenant")
                        .WithMany("Roles")
                        .HasForeignKey("TenantId")
                        .IsRequired()
                        .HasConstraintName("FK__Roles__TenantId__5BE2A6F2");

                    b.Navigation("Tenant");
                });

            modelBuilder.Entity("Host.Infrastructure.Models.Room", b =>
                {
                    b.HasOne("Host.Infrastructure.Models.Property", "Property")
                        .WithMany("Rooms")
                        .HasForeignKey("PropertyId")
                        .IsRequired()
                        .HasConstraintName("FK__Rooms__PropertyI__72C60C4A");

                    b.Navigation("Property");
                });

            modelBuilder.Entity("Host.Infrastructure.Models.RoomAvailability", b =>
                {
                    b.HasOne("Host.Infrastructure.Models.Room", "Room")
                        .WithMany("RoomAvailabilities")
                        .HasForeignKey("RoomId")
                        .IsRequired()
                        .HasConstraintName("FK__RoomAvail__RoomI__7E37BEF6");

                    b.Navigation("Room");
                });

            modelBuilder.Entity("Host.Infrastructure.Models.RoomImage", b =>
                {
                    b.HasOne("Host.Infrastructure.Models.Room", "Room")
                        .WithMany("RoomImages")
                        .HasForeignKey("RoomId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("FK__RoomImage__RoomI__778AC167");

                    b.Navigation("Room");
                });

            modelBuilder.Entity("Host.Infrastructure.Models.Technician", b =>
                {
                    b.HasOne("Host.Infrastructure.Models.Tenant", "Tenant")
                        .WithMany("Technicians")
                        .HasForeignKey("TenantId")
                        .IsRequired()
                        .HasConstraintName("FK__Technicia__Tenan__3D2915A8");

                    b.Navigation("Tenant");
                });

            modelBuilder.Entity("Host.Infrastructure.Models.Tenant", b =>
                {
                    b.HasOne("Host.Infrastructure.Models.SubscriptionPackage", "Package")
                        .WithMany("Tenants")
                        .HasForeignKey("PackageId")
                        .IsRequired()
                        .HasConstraintName("FK__Tenants__Package__534D60F1");

                    b.Navigation("Package");
                });

            modelBuilder.Entity("Host.Infrastructure.Models.User", b =>
                {
                    b.HasOne("Host.Infrastructure.Models.Tenant", "Tenant")
                        .WithMany("Users")
                        .HasForeignKey("TenantId")
                        .IsRequired()
                        .HasConstraintName("FK__Users__TenantId__5812160E");

                    b.Navigation("Tenant");
                });

            modelBuilder.Entity("Host.Infrastructure.Models.UserAccessToken", b =>
                {
                    b.HasOne("Host.Infrastructure.Models.User", "User")
                        .WithMany("UserAccessTokens")
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("FK_UserAccessTokens_Users_UserId");

                    b.Navigation("User");
                });

            modelBuilder.Entity("Host.Infrastructure.Models.UserRefreshToken", b =>
                {
                    b.HasOne("Host.Infrastructure.Models.User", "User")
                        .WithMany("UserRefreshTokens")
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("FK_UserRefreshTokens_Users_UserId");

                    b.Navigation("User");
                });

            modelBuilder.Entity("RolePermission", b =>
                {
                    b.HasOne("Host.Infrastructure.Models.Permission", null)
                        .WithMany()
                        .HasForeignKey("PermissionId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("FK__RolePermi__Permi__628FA481");

                    b.HasOne("Host.Infrastructure.Models.Role", null)
                        .WithMany()
                        .HasForeignKey("RoleId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("FK__RolePermi__RoleI__619B8048");
                });

            modelBuilder.Entity("UserPropertyMapping", b =>
                {
                    b.HasOne("Host.Infrastructure.Models.Property", null)
                        .WithMany()
                        .HasForeignKey("PropertyId")
                        .IsRequired()
                        .HasConstraintName("FK__UserPrope__Prope__6E01572D");

                    b.HasOne("Host.Infrastructure.Models.User", null)
                        .WithMany()
                        .HasForeignKey("UserId")
                        .IsRequired()
                        .HasConstraintName("FK__UserPrope__UserI__6D0D32F4");
                });

            modelBuilder.Entity("UserRole", b =>
                {
                    b.HasOne("Host.Infrastructure.Models.Role", null)
                        .WithMany()
                        .HasForeignKey("RoleId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("FK__UserRoles__RoleI__66603565");

                    b.HasOne("Host.Infrastructure.Models.User", null)
                        .WithMany()
                        .HasForeignKey("UserId")
                        .OnDelete(DeleteBehavior.Cascade)
                        .IsRequired()
                        .HasConstraintName("FK__UserRoles__UserI__656C112C");
                });

            modelBuilder.Entity("Host.Infrastructure.Models.Asset", b =>
                {
                    b.Navigation("AssetAssignments");
                });

            modelBuilder.Entity("Host.Infrastructure.Models.AssetAssignment", b =>
                {
                    b.Navigation("MaintenanceTickets");
                });

            modelBuilder.Entity("Host.Infrastructure.Models.Bed", b =>
                {
                    b.Navigation("AssetAssignments");

                    b.Navigation("Bookings");

                    b.Navigation("Contracts");

                    b.Navigation("Listings");
                });

            modelBuilder.Entity("Host.Infrastructure.Models.Booking", b =>
                {
                    b.Navigation("Contracts");

                    b.Navigation("Invoices");
                });

            modelBuilder.Entity("Host.Infrastructure.Models.Broker", b =>
                {
                    b.Navigation("Contracts");
                });

            modelBuilder.Entity("Host.Infrastructure.Models.Contract", b =>
                {
                    b.Navigation("Invoices");
                });

            modelBuilder.Entity("Host.Infrastructure.Models.Listing", b =>
                {
                    b.Navigation("ListingImages");
                });

            modelBuilder.Entity("Host.Infrastructure.Models.Property", b =>
                {
                    b.Navigation("Bookings");

                    b.Navigation("Contracts");

                    b.Navigation("Listings");

                    b.Navigation("Rooms");
                });

            modelBuilder.Entity("Host.Infrastructure.Models.Resident", b =>
                {
                    b.Navigation("Contracts");

                    b.Navigation("MaintenanceTickets");
                });

            modelBuilder.Entity("Host.Infrastructure.Models.Room", b =>
                {
                    b.Navigation("AssetAssignments");

                    b.Navigation("Beds");

                    b.Navigation("Bookings");

                    b.Navigation("Contracts");

                    b.Navigation("Listings");

                    b.Navigation("MaintenanceTickets");

                    b.Navigation("RoomAvailabilities");

                    b.Navigation("RoomImages");
                });

            modelBuilder.Entity("Host.Infrastructure.Models.SubscriptionPackage", b =>
                {
                    b.Navigation("Tenants");
                });

            modelBuilder.Entity("Host.Infrastructure.Models.Technician", b =>
                {
                    b.Navigation("MaintenanceTickets");
                });

            modelBuilder.Entity("Host.Infrastructure.Models.Tenant", b =>
                {
                    b.Navigation("AssetAssignments");

                    b.Navigation("Assets");

                    b.Navigation("Bookings");

                    b.Navigation("Brokers");

                    b.Navigation("Contracts");

                    b.Navigation("Invoices");

                    b.Navigation("Listings");

                    b.Navigation("MaintenanceTickets");

                    b.Navigation("Properties");

                    b.Navigation("Residents");

                    b.Navigation("Roles");

                    b.Navigation("Technicians");

                    b.Navigation("Users");
                });

            modelBuilder.Entity("Host.Infrastructure.Models.User", b =>
                {
                    b.Navigation("UserAccessTokens");

                    b.Navigation("UserRefreshTokens");
                });
#pragma warning restore 612, 618
        }
    }
}
